...
import { generateProfePlanStream, generateCanvaData } from './services/geminiService'; // NOVO
...
import CanvaExportModal from './components/CanvaExportModal'; // NOVO
...
const App: React.FC = () => {
  ...
  const [presentationTheme, setPresentationTheme] = useState(''); // Input for Presentations mode request
  ...
  const handleGenerateCanvaData = async () => {
    if (!messages.length) return;
    setIsThinking(true);
    try {
      const lastContent = [...messages].reverse().find(m => m.role === MessageRole.ASSISTANT && m.content.length > 100)?.content;
      if (!lastContent && !presentationTheme) throw new Error("Gere um conteúdo primeiro ou defina um tema.");

      const sourceText = lastContent || `Tema: ${presentationTheme}`;
      const data = await generateCanvaData(sourceText);
      setPresentationScript(data);
      setIsPresentationModalOpen(true);
    } catch (e) {
      setError("Erro ao gerar dados Canva: " + e);
    } finally {
      setIsThinking(false);
    }
  };
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    let currentMsg = input;

    // Injeção de contexto dos novos inputs
    if (activeMode === ToolMode.QUARTERLY_PLANNING && quarter) {
      currentMsg += `\n[Contexto]: Planejamento para o ${quarter}.`;
    } else if (activeMode === ToolMode.ENEM_BANK && enemArea) {
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
                ...
                {messages.map((msg) => (
                  <div key={msg.id} className={`flex ${msg.role === MessageRole.USER ? 'justify-end' : 'justify-start'} mb-10`}>
                    <div className={`flex items-start gap-5 max-w-[85%]`}>
                      ...
                      <div className={`p-6 rounded-[2.5rem] shadow-2xl relative transition-all ${msg.role === MessageRole.USER ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white border border-slate-100 rounded-tl-none'
                        }`}>
                  ...
                  <div className="flex justify-start mb-10">
                    <div className="flex items-center gap-4 bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100 shadow-sm animate-pulse">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-bold uppercase">Thinking Mode Ativado</p>
                      </div>
                    </div>
                  </div>
                )}
      ...
      <aside className="h-screen bg-white border-l border-slate-100 flex-col shrink-0 lg:flex lg:w-72 p-10 space-y-10 overflow-y-auto hidden">
        {activeMode === ToolMode.PRESENTATIONS ? (
          <>
            <div>
              <h3 className="font-black text-[10px] uppercase tracking-[0.2em] text-slate-400 italic mb-8">Integração Canva</h3>
              <div className="space-y-4">
                <button
                  onClick={handleGenerateCanvaData}
                  disabled={isThinking}
                  className="w-full p-6 bg-gradient-to-br from-purple-600 to-indigo-700 text-white rounded-[2rem] shadow-xl shadow-purple-200 hover:scale-105 transition-all active:scale-95 group relative overflow-hidden"
                >
                  <div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                  <div className="flex flex-col items-center gap-2">
                    <Sparkle size={24} className="animate-pulse" />
                    <span className="font-black text-xs uppercase tracking-widest text-center">Exportar Tabela (Canva)</span>
                  </div>
                </button>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center justify-center gap-4 w-full p-5 bg-blue-50 text-blue-700 rounded-2xl font-black uppercase text-[10px] border border-blue-100 hover:bg-blue-100 transition-all active:scale-95 shadow-sm group"
                >
                  <ExternalLink size={18} className="group-hover:translate-x-0.5 transition-transform" /> Ir para Canva for Education
                </a>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] ml-1">Tema / Briefing</label>
                <textarea
                  value={presentationTheme} onChange={(e) => setPresentationTheme(e.target.value)}
                  placeholder="Ex: Apresentação de Biologia sobre Células, estilo minimalista..."
                  rows={6}
                  className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-black outline-none focus:ring-2 focus:ring-blue-100 focus:bg-white transition-all shadow-inner resize-none"
                />
              </div>
            </div>
          </>
              ...
              <div className="space-y-6">
                ...
                {(activeMode === ToolMode.QUARTERLY_PLANNING) && (
                  <div className="space-y-2 animate-in slide-in-from-left-2 fade-in duration-300">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] ml-1">Trimestre / Bimestre</label>
                    <input
                      type="text" value={quarter} onChange={(e) => setQuarter(e.target.value)}
                ...
                {(activeMode === ToolMode.ENEM_BANK) && (
                  <div className="space-y-2 animate-in slide-in-from-left-2 fade-in duration-300">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] ml-1">Área do Conhecimento</label>
                    <select
                      value={enemArea} onChange={(e) => setEnemArea(e.target.value)}
                      className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-black outline-none focus:ring-2 focus:ring-blue-100 focus:bg-white transition-all shadow-inner appearance-none bg-no-repeat bg-[right_1rem_center]"
                    >
