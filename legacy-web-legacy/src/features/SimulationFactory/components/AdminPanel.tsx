...
import { Settings, Database, BarChart3, Upload, Trash2, Eye, Edit, Plus, RefreshCw, Download } from 'lucide-react';
import {
    questionBank,
    simulationCache,
    simulationAnalytics,
    ...
    <div>
        ...
        {questions.length === 0 ? (
            <div className="text-center py-12 text-slate-400">
                Digite uma query para buscar questões
            </div>
        ) : (
            ...
            <div className="space-y-2">
                {analytics.topQueries.slice(0, 10).map((item, idx) => (
                    <div key={idx} className="flex items-center justify-between text-sm">
                        <span className="text-white">{idx + 1}. {item.query}</span>
