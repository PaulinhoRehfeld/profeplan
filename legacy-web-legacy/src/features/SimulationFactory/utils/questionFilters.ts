...
export const filterByArea = (
    ...
    const targetDisciplines = areas.flatMap(area =>
        AREA_DISCIPLINE_MAP[area as QuestionArea] || []
    );
 ...
 */
export const filterCompleteQuestions = (
    questions: SimulationQuestion[]
): SimulationQuestion[] => {
...
export const getQuestionPreview = (
    ...
    const text = question.metadata?.alternativesIntroduction ||
    ...
    questions.forEach(q => {
        ...
        for (const [area, disciplines] of Object.entries(AREA_DISCIPLINE_MAP)) {
            const isMatch = disciplines.some(d => {
                ...
                return normalizedDisc.includes(normalizedTarget) || normalizedTarget.includes(normalizedDisc);
            });
