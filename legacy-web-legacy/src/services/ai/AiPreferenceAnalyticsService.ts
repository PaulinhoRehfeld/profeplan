 ...
 */
export async function logPreferenceUsage(log: Omit<PreferenceUsageLog, 'id' | 'generated_at'>): Promise<string | null> {
    try {
 ...
 */
export async function getPreferenceAnalytics(userId: string, daysLookback: number = 30): Promise<PreferenceAnalytics | null> {
    try {
        ...
        analytics.overall_satisfaction = satisfactionCount > 0
...
export function getPreferenceSuggestions(analytics: PreferenceAnalytics): string[] {
    ...
    if (methodologies.length > 1) {
        ...
        if (best[1].avg_satisfaction > 4.0 && best[1].usage_count >= 3) {
            suggestions.push(`✨ Sua metodologia "${best[0]}" tem ${best[1].avg_satisfaction.toFixed(1)}⭐ de satisfação média. Continue usando!`);
        }
    }
    ...
    methodologies.forEach(([name, stats]) => {
        if (stats.regeneration_rate > 40 && stats.usage_count >= 5) {
            suggestions.push(`⚠️ "${name}" tem ${stats.regeneration_rate.toFixed(0)}% de taxa de regeneração. Considere mudar de metodologia.`);
        }
    });

    // Satisfação geral baixa
    if (analytics.overall_satisfaction < 3.0 && analytics.total_generations >= 10) {
        suggestions.push(`💡 Sua satisfação média está em ${analytics.overall_satisfaction.toFixed(1)}⭐. Experimente ajustar suas preferências em Configurações.`);
    }

    // Satisfação geral alta
    if (analytics.overall_satisfaction >= 4.5 && analytics.total_generations >= 10) {
        suggestions.push(`🎉 Excelente! Suas preferências atuais geram ${analytics.overall_satisfaction.toFixed(1)}⭐ de satisfação média.`);
    }
