...
export const parseClassListFromText = async (rawText: string) => {
    ...
    const subjectLine = lines.find((line) => normalizeText(line).includes('COMPONENTE:') || normalizeText(line).includes('DISCIPLINA:') || normalizeText(line).includes('COMPONENTE CURRICULAR:'));

    const extractClassName = () => {
        if (!classLine) return null;
        const normalized = normalizeText(classLine);
        const match = normalized.match(/TURMA:\s*(?:TURNO:\s*)?(?:PROFESSOR:\s*)?(\d+)\s*-\s*(.+?)(?:\s*-\s*|\s+)(?:\(\d{4}\))?/i);
        if (match) return match[2].replace(/\s+/g, ' ').trim();
        const afterTurma = classLine.split('Turma:')[1];
        if (!afterTurma) return null;
        const cleaned = afterTurma.replace(/TURNO:.*$/i, '').replace(/PROFESSOR:.*$/i, '').trim();
