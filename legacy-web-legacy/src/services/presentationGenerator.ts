...
export const generateHighFidelityPPT = async (slideData: SlideStructure[], topic: string) => {
    ...
    pres.defineSlideMaster({
        ...
        objects: [
            ...
            {
                placeholder: {
                    options: { name: "title", type: "title", x: 0.7, y: 0.5, w: "85%", h: 0.6, align: "left", color: "FFFFFF", fontSize: 24, bold: true }
                }
            },
            // Área de texto à esquerda
            {
                placeholder: {
                    options: { name: "content", type: "body", x: 0.7, y: 1.5, w: "45%", h: 5.0, fontSize: 18, color: "333333", bullet: true }
                }
            },
            // Área de imagem à direita (placeholder visual)
            { rect: { x: 5.5, y: 1.5, w: 4.5, h: 4.0, fill: { color: "E0E0E0" }, line: { color: "CCCCCC", width: 1 } } },
        ],
    });
    ...
    for (const slide of slideData) {
        ...
        switch (slide.type) {
            case 'TITLE_SLIDE':
                ...
                slideRef.addText(slide.title, { x: 1, y: 2.5, w: "80%", h: 1.5, fontSize: 44, color: "FFFFFF", bold: true, align: "center" });
                slideRef.addText(topic.toUpperCase(), { x: 1, y: 4, w: "80%", h: 0.5, fontSize: 20, color: "FFD700", align: "center" });
                break;
            ...
            case 'CONTENT_IMAGE_RIGHT':
                ...
                if (slide.imageSearchQuery) {
                    ...
                    if (image) {
                        ...
                        slideRef.addText(`Foto por ${image.credit} no Unsplash`, { x: 5.5, y: 5.6, fontSize: 9, color: "666666" });
                    }
                }
                break;
            ...
            default:
                slideRef = pres.addSlide();
                slideRef.addText(slide.title, { x: 0.5, y: 0.5, w: "90%", h: 0.8, fontSize: 30, color: "0056b3", bold: true });
                slideRef.addText(slide.contentPoints, { x: 0.5, y: 1.5, w: "90%", h: 5.0, fontSize: 18, color: "333333", bullet: true });
                break;
        }
    }
