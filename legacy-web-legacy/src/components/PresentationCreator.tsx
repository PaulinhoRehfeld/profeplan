...
import {
    Presentation, MonitorPlay, Sparkles, RefreshCw, Layers,
...
const PresentationCreator: React.FC<PresentationCreatorProps> = ({ userId, setSidebarContent }) => {
                ...
                <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
                    <div className="bg-gradient-to-br from-fuchsia-50 to-purple-50 border border-fuchsia-100 rounded-[2.5rem] p-6 shadow-lg">
                        ...
                        <div className="space-y-3">
                            ...
                            >
                                <MonitorPlay size={18} className="group-hover:translate-x-0.5 transition-transform" />
                                Apresentar Agora
                            </button>
                    ...
                    <div className="bg-slate-900 rounded-[2.5rem] p-6 shadow-xl text-white relative overflow-hidden">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="bg-white/5 p-3 rounded-2xl text-center">
                                <p className="text-[9px] text-slate-400 uppercase tracking-widest mb-1">Tempo Est.</p>
                                <p className="text-xl font-black text-fuchsia-400">~{generatedPresentation.slides.length * 2}min</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-6 animate-in fade-in duration-500">
                    <div className="bg-white border border-slate-100 rounded-[2.5rem] p-6 shadow-sm">
                        ...
                        <p className="text-xs text-slate-500 font-medium leading-relaxed">
                            Configure os parâmetros ao lado para gerar sua aula visual. O assistente criará a estrutura completa para exportação.
                        </p>
                    </div>
                </div>
            );
    ...
    const handleSavePresentation = async () => {
        ...
        try {
            const contentMarkdown = generatedPresentation.slides.map((s: any) =>
        ...
        return (
            <div className="fixed inset-0 bg-slate-900 z-50 flex flex-col items-center justify-center text-white">
                ...
                <div className="w-full max-w-6xl aspect-video bg-white text-slate-900 rounded-3xl p-16 shadow-2xl relative overflow-hidden flex flex-col">
                    {/* Background Decor */}
                    <div className={`absolute top-0 right-0 w-96 h-96 rounded-full blur-3xl opacity-30 pointer-events-none -mr-20 -mt-20 ${visualStyle.includes('Lúdico') ? 'bg-yellow-400' : 'bg-fuchsia-600'}`}></div>
                    ...
                    <div className="mb-12 relative z-10 flex justify-between items-start">
                        <div>
                            ...
                            <h1 className="text-5xl font-black text-slate-900 leading-tight max-w-4xl">{slide.title}</h1>
                        </div>
                        ...
                        <div className="space-y-6">
                            <ul className="space-y-4">
                                {slide.contentBulletPoints?.map((pt: string, i: number) => (
                                    <li key={i} className="text-2xl font-medium text-slate-700 flex gap-4 leading-relaxed">
                    ...
                    <div className="mt-8 pt-6 border-t border-slate-100">
                        <p className="text-sm text-slate-400 font-medium italic">
                            <span className="font-bold text-slate-500 not-italic uppercase tracking-wider mr-2">Roteiro:</span>
                            {slide.speakerNotes}
                        </p>
                    </div>
                </div>
                ...
                <div className="absolute bottom-8 flex items-center gap-6">
                    ...
                    <span className="text-xl font-black tracking-widest">{currentSlideIndex + 1} / {generatedPresentation.slides.length}</span>
                    <button
            ...
            <div className="w-full max-w-6xl mx-auto space-y-8 animate-in slide-in-from-bottom-8 duration-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {generatedPresentation.slides.map((slide: any, idx: number) => (
                        <div key={idx} className="bg-white border border-slate-200 rounded-[2rem] overflow-hidden shadow-sm hover:shadow-xl transition-all group hover:-translate-y-1 cursor-pointer" onClick={() => { setCurrentSlideIndex(idx); setIsPresenting(true); }}>
                            {/* Header do Slide */}
                            ...
                            <div className="p-6 h-64 overflow-y-auto custom-scrollbar">
                                <h4 className="font-bold text-lg text-slate-800 mb-4 leading-tight">{slide.title}</h4>
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in duration-500">
            <div className="text-center space-y-4">
                ...
                <h2 className="text-3xl font-black text-slate-900 tracking-tight uppercase italic">Criar Apresentação de Impacto</h2>
                <p className="text-sm font-medium text-slate-500 max-w-lg mx-auto">
                    Transforme suas ideias em slides visuais automaticamente. Selecione o tema, configure o estilo e deixe a IA estruturar sua aula.
                </p>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-[3rem] p-10 shadow-xl shadow-slate-200/50 relative overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-6">
                        <div className="space-y-3">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Tema ou Assunto da Aula</label>
                                ...
                                {recentLessons.length > 0 && (
                                    ...
                                    >
                                        ...
                                        {recentLessons.map((l, i) => (
                                            <option key={i} value={l.topic}>{l.topic}</option>
                                        ))}
                                    </select>
                                )}
                                <input
                        ...
                        <div className="space-y-3">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Quantidade de Slides</label>
                            <div className="flex items-center gap-4 bg-slate-50 p-2 rounded-2xl border border-slate-200">
                                <input
                                    type="range" min="5" max="20" step="1"
                                    value={slideCount} onChange={(e) => setSlideCount(Number(e.target.value))}
                                    className="flex-1 h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-purple-600 ml-4"
                                />
                                <span className="w-12 h-10 flex items-center justify-center bg-white rounded-xl font-black text-purple-600 text-sm shadow-sm">{slideCount}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        <div className="space-y-3">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Estilo Visual</label>
                            <div className="grid grid-cols-2 gap-3">
                                {['Moderno', 'Lúdico', 'Corporativo', 'Minimalista'].map(s => (
                                    <button
                                        key={s}
                                        onClick={() => setVisualStyle(s)}
                                        className={`p-3 rounded-2xl text-xs font-bold border-2 transition-all ${visualStyle === s
                        ...
                        <div className="flex items-center justify-between bg-slate-50 p-4 rounded-2xl border border-slate-200">
                            <div className="flex items-center gap-3">
                                <div className="bg-white p-2 rounded-xl text-fuchsia-500 shadow-sm"><Sparkles size={16} /></div>
                                <div>
                            ...
                            >
                                <div className={`w-5 h-5 bg-white rounded-full shadow-md transform transition-transform ${includeInteractions ? 'translate-x-5' : 'translate-x-0'}`} />
                            </button>
                        </div>
                    </div>
                </div>
