...
export const SchoolDashboard: React.FC<SchoolDashboardProps> = ({
  ...
  const loadDashboardData = async (schoolId: string) => {
    ...
    try {
      ...
      console.log(
        '[SchoolDashboard] 🔍 Loading teachers from teacher_schools for school:',
        resolvedSchoolId
      );
      const teachersData = await getActiveTeachersBySchool(resolvedSchoolId);

      console.log(
        '[SchoolDashboard] ✅ Found',
        teachersData.length,
        'active teachers via teacher_schools'
      );
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full p-8 text-center bg-slate-50">
        ...
        <p className="text-slate-500 max-w-md mb-8">
          Você está acessando como <strong>Gestor Escolar</strong>, mas sua conta ainda não está
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm max-w-md w-full text-left">
          ...
          <ul className="space-y-3 text-sm text-slate-600 mb-6">
            <li className="flex items-start gap-2">
              <span className="w-5 h-5 bg-slate-100 rounded-full flex items-center justify-center text-xs font-bold shrink-0 mt-0.5">
                1
              </span>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                >
                  <option value="">
                    {isSearchingSchools ? 'Carregando...' : 'Selecione a Escola...'}
                  </option>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8">
        ...
        <div className="flex space-x-1 bg-white rounded-lg p-1 shadow-sm border border-slate-200 mb-6 w-full max-w-md mx-auto md:mx-0">
          <button
            ...
            className={`flex-1 px-4 py-2.5 rounded-md text-sm font-bold transition ${
              activeTab === 'teachers'
