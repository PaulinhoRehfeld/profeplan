...
export const hybridSearchProfeplan = async ({
  ...
  try {
    ...
    const needsHydration = finalResults.some(
      (q) => !q.metadata || !(q.metadata.alternatives as unknown[] | undefined)?.length
    );
...
export const searchCurriculum = async (
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        `[searchService] searchProxy retornou ${response.status} — usando fallback direto.`
      );
