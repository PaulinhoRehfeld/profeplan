...
export const getUserProfile = async (
  ...
  try {
    ...
    if (rpcUnavailable) {
      console.warn(
        '[userService] get_my_profile RPC not deployed — run 20260624_get_my_profile_rpc.sql in Supabase.'
      );
    }
    ...
    if (activeAuthUser?.id && activeAuthUser.id !== userId) {
      console.warn(
        `[userService] Session ID mismatch detected. requested=${userId} auth=${activeAuthUser.id}. Using auth ID.`
      );
    }
    ...
    if ((error || !data) && activeEmail) {
      console.warn(
        `[userService] Profile not found for ID ${lookupUserId}. Attempting fallback by email: ${activeEmail}`
      );
    ...
    if (error || !data) {
      ...
      if (error?.code === '42501' || status === 403) {
        console.error(
          "[userService] ⛔ RLS PERMISSION DENIED. Check Supabase Policies for 'profiles' table."
        );
      }
      ...
      try {
        ...
        if (!authUser) {
          ...
          if (authUser.id !== lookupUserId) {
            console.warn(
              `[userService] Ghost ID detected: stored=${lookupUserId} auth=${authUser.id}. Using auth ID.`
            );
          }
            ...
            .upsert(
              {
                id: targetId,
                email: fallbackEmail,
                full_name: userMetaName || fallbackEmail.split('@')[0],
...
export const updateUserProfile = async (
  ...
  try {
    ...
    if (
      rpcError &&
      /autenticado|auth|jwt|token|expir|session|bearer/i.test(rpcError.message || '')
    ) {
      console.warn(
        '[userService] 🔄 RPC update_my_profile falhou com erro de auth, tentando refresh de sessão...'
      );
      try {
        const {
          data: { session },
    ...
    if (!rpcError && !rpcData) {
      console.error(
        '[userService] ⚠️ RPC update_my_profile retornou NULL (0 linhas afetadas) — perfil pode não existir ou Ghost ID não resolvido.'
      );
    ...
    if (fnMissing) {
      console.error(
        '[userService] ❌ RPC update_my_profile NÃO ENCONTRADA — execute a migration 20260709_fix_production_profile_save.sql no Supabase.'
      );
    } else {
      console.error(
        '[userService] ❌ RPC update_my_profile FALHOU:',
    ...
    try {
      ...
      if (user?.id) {
        if (user.id !== userId) {
          console.warn(
            `[userService] updateUserProfile: id divergente (param=${userId} auth=${user.id}). Usando auth id.`
          );
        }
        targetId = user.id;
      }
    } catch {
      /* segue com o userId recebido */
    }
    ...
    if (!updatedRows || updatedRows.length === 0) {
      console.warn(
        '[userService] Update afetou 0 linhas (Ghost ID provável) — buscando perfil via get_my_profile RPC...'
      );
      ...
      console.warn(
        '[userService] get_my_profile RPC falhou ou retornou vazio — tentando upsert de recuperação.'
      );
      ...
      if (upsertErr || !upserted) {
        ...
        return {
          ...
          error:
            upsertErr?.message ||
            'As alterações não foram salvas. Verifique se sua sessão está ativa e tente novamente.',
        };
      }
    ...
    return {
      success: false,
      error: getErrorMessage(err) || 'Erro fatal ao conectar com o banco de dados',
    };
  }
};
...
export const hasFeaturePattern = (
