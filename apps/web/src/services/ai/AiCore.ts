...
export const GENERATION_MODELS = ['backend-ai-proxy'];
...
export async function createSimpleCompletion(
  ...
  return typeof content === 'string'
    ? content
    : ((content as { text?: string }[] | undefined)?.map((c) => c?.text ?? '').join('') ?? '');
}
...
export async function executeWithFallback<T>(
  ...
  throw new Error(
    `Todas as tentativas de deployment/model falharam para ${actionName}. Último erro: ${getErrorMessage(lastError)}`
  );
}
