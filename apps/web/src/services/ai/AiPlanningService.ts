...
import {
  extractGuardrailsFromSettings,
  generateGuardrailsPrompt,
  AIGuardrailsConfig,
} from './AiGuardrailsService';
...
export const generateTermPlan = async (
  ...
  try {
    ...
    console.log(
      `🔍 Buscando currículo para: ${normalizedSubject}, ${normalizedGrade}, ${periodString}`
    );
    ...
    if (context.stateBase?.includes('Minas Gerais') || context.stateBase === 'MG') {
      const seeCurriculum = await getDeterministicCurriculum(
        normalizedSubject,
        periodString,
        normalizedGrade
      );
  ...
  let pnldInstruction = '';
  if (
    context.pnld_book_id &&
    typeof context.pnld_book_id === 'string' &&
    (context.pnld_book_id.toLowerCase().includes('educação digital') ||
      context.pnld_book_id.toLowerCase().includes('educacao digital'))
  ) {
  ...
  const extractedCodes = curriculumContext
    ? [...curriculumContext.matchAll(/\b(E[MF]\d{2}[A-Z]{2,5}\d{2,4}[A-Z0-9]*)\b/g)].map(
        (m) => m[1]
      )
    : [];
  ...
  const prompt = `
    ...
    **Conteúdo Abrangido:**
    ...
    ${
      termExamEnabled
        ? `### Aula ${penultimateClass}: PROVA 02
    **Descrição:**
...
export const generateGeminiContent = async (
  ...
  const response =
    typeof completion.choices[0]?.message?.content === 'string'
      ...
      : ((completion.choices[0]?.message?.content as any[] | undefined)
