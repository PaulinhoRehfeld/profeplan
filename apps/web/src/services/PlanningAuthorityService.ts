...
export const PlanningAuthority = {
  ...
  checkKnowledgeBase: async (intent: PlanningIntent): Promise<boolean> => {
    ...
    const isMGEM =
      intent.level === 'Ensino Médio' &&
      (intent.grade.includes('EM') || parseInt(intent.grade) <= 3);
  ...
  executePlanning: async (intent: PlanningIntent) => {
    ...
    if (!hasKnowledge) {
      ...
      console.warn(
        `⚠️ GOVERNANCE ALERT: Sem currículo base para ${safeIntent.subject} - ${safeIntent.grade}`
      );
    }
...
PERFIL: Você fala como um Coordenador Pedagógico experiente conversando com um Professor.`;
    ...
    const history = ((context?.history as Array<{ role: string; content: string }>) || []).map(
      (h) => ({
  ...
  getKnowledgeManifest: async () => {
    ...
    try {
      const { data, error } = await supabase.from('curriculos_mg').select('metadata');
