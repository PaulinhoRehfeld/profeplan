 ...
 */
export const saveClassToLocal = (
  userId: string,
  classData: { className: string; subject: string; students: string[] }
): LocalClass => {
 ...
 */
export const saveLessonToLocal = (
  userId: string,
  topic: string,
  content: string,
  canvaData: unknown
...
export const getLessonsByClass = (classId: string): LocalLesson[] => {
  ...
  try {
    ...
    return allLessons
      .filter((lesson) => lesson.topic?.includes(classId) || lesson.content?.includes(classId))
