...
export async function logEventForClass(
    ...
    try {
        ...
        if (!schoolId) {
            console.warn(`[pdiRecordRepository] logEventForClass: school_id não resolvido para a turma ${classId}. Automação de PDI ignorada (${students.length} alunos).`);
            return [];
        }

        return Promise.all(
            students.map(s => logEvent(s.id, type, title, content, pdiBlock, schoolId, classId, teacherId))
        );
    } catch (error) {
