...
export function mapToCompatibility(
  ...
  return {
    ...
    blocks_completed: {
      ...
      block_9: Array.isArray(pdi.block_9_content)
        ? (pdi.block_9_content as unknown[]).length > 0
        : false,
      block_10: Array.isArray(pdi.block_10_entries)
        ? (pdi.block_10_entries as unknown[]).length > 0
        : false,
...
export function calculateCompleteness(pdi: PdiDocument): PdiCompleteness {
  ...
  if (content.clinical_health && Object.keys(content.clinical_health).length > 0)
    completed.push('Dados Clínicos');
  else missing.push('Dados Clínicos');

  if (content.psychomotor && Object.keys(content.psychomotor).length > 5)
    completed.push('Psicomotor');
  ...
  if (content.communication && Object.keys(content.communication).length > 0)
    completed.push('Comunicação');
