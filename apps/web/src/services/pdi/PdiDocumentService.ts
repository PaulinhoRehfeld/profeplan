...
import {
  mapToCompatibility as _mapToCompatibility,
  calculateCompleteness as _calculateCompleteness,
} from './pdiUtils';
  ...
  async exportPdiToDocx(pdi: PdiDocument): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      return {
        success: false,
        error:
          'Erro ao exportar PDI para DOCX. Verifique se bibliotecas "docx" e "file-saver" estão instaladas.',
      };
    }
  },
};
