...
export const checkUsageQuota = async (
  ...
  if (!profile) {
    ...
    try {
      ...
      try {
        ...
        if (user?.id) {
          console.warn(
            `[quota] Profile null mas getUser confirma sessão (${user.id}) — permitindo.`
          );
          return { allowed: true };
        }
      } catch {
    ...
    if (networkError) {
      ...
      return {
        allowed: false,
        message:
          'Falha de conexão com o servidor. Verifique sua internet e tente novamente em instantes.',
      };
    }
  ...
  if (profile.credits === null || profile.credits === undefined) {
    console.warn(
      `[quota] credits is null for profile ${profile.id} — allowing (ghost-id migration state).`
    );
    return { allowed: true };
  }
...
export const incrementUserUsage = async (
  ...
  if (profile.credits <= 0) {
    console.warn(
      `[userService] Attempted to deduct from empty balance for user ${userId}. Task: ${taskType}`
    );
    return;
  }
