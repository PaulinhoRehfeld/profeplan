...
export { resolveAuthUid }; // Re-export for backward compatibility (era local a este módulo)
...
export const getTeacherContext = async (userId: string, limit: number = 3) => {
  ...
  const { data: profile } = await supabase.from('profiles').select('*').eq('id', authUid).single();
...
export const saveClassStructure = async (
  ...
  classData: {
    ...
    subject: string;
    students: Array<
      string | { name: string; student_code?: string; call_number?: number; observations?: string }
    >;
    schoolId?: string;
  }
) => {
  ...
  const { error: studentError } = await supabase.from('students').insert(studentRows);
...
export const updateLessonTracking = async (
  ...
  termPlanId: string,
  lessonIndex: number,
  status: 'pending' | 'prepared' | 'taught' = 'prepared'
) => {
  // Usa auth.uid() real — evita ghost UUID no RLS (mesmo padrão de getClasses)
  let authUid: string;
  try {
  ...
  const { data, error } = await supabase.from('lesson_tracking').upsert(
    {
      user_id: authUid,
