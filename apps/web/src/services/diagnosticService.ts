...
export const runDiagnostics = async (): Promise<DiagnosticResult[]> => {
  ...
  if (!supabaseUrl || !supabaseKey) {
    results.push({
      service: 'EnvVars',
      status: 'error',
      message: 'Variáveis Supabase ausentes. Verifique VITE_SUPABASE_URL e VITE_SUPABASE_ANON_KEY.',
    });
  } else {
    ...
    try {
      ...
      if (error) {
        results.push({
          ...
          latency: endSupabase - startSupabase,
        });
      } else {
        results.push({
          service: 'Supabase',
      ...
      results.push({
        service: 'Supabase',
        status: 'error',
        message: `Exceção ao conectar: ${getErrorMessage(e)}`,
      });
    }
  }
    ...
    if (response.ok) {
      ...
      if (data.embedding && Array.isArray(data.embedding)) {
        ...
        results.push({
          ...
          status: 'error',
          message:
            'Backend Gemini API respondeu com sucesso, mas o formato do embedding é inválido.',
          latency: endGemini - startGemini,
        });
      }
    } else {
