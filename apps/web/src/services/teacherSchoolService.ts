...
export const updateTeacherSchoolLink = async (
  ...
  try {
    const { error } = await supabase.from('teacher_schools').update(updates).eq('id', linkId);
 ...
 */
export const endTeacherSchoolLink = async (
  linkId: string
): Promise<{ success: boolean; error?: string }> => {
  try {
 ...
 */
export const clearTeacherSchoolLinks = async (
  teacherId: string
): Promise<{ success: boolean; error?: string }> => {
  try {
    const { error } = await supabase
...
export const reconcileTeacherByInep = async (
  ...
  try {
    ...
    console.log(
      '[teacherSchoolService] ✅ INEP normalized:',
      `"${inepCode}" → "${normalizedInep}"`
    );

    // 1. Buscar escola pelo INEP normalizado
    console.log(
      '[teacherSchoolService] Searching for INEP:',
      `"${normalizedInep}"`,
      'Length:',
    ...
    if (!currentProfile?.active_school_id) {
      ...
      const { error: updateError } = await supabase.rpc('set_my_active_school', {
      ...
      console.log(
        '[teacherSchoolService] ℹ️ active_school_id already set, not overwriting:',
        currentProfile.active_school_id
      );
    }
 ...
 */
export const getSchoolManagers = async (
  schoolId: string
): Promise<{
  success: boolean;
  managers?: Array<{
...
export const getTeacherSchoolManager = async (
  ...
  try {
    ...
    if (
      ...
      managersResult.managers.length === 0
    ) {
      return {
        success: false,
