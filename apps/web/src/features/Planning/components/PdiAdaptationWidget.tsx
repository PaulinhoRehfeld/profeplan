...
export const PdiAdaptationWidget: React.FC<PdiAdaptationWidgetProps> = ({
    userId, studentId, pdiId, studentName, lessonTopic, lessonObjective
}) => {
    // State
    ...
    const [achievedStatus, setAchievedStatus] = useState<'ALCANCADO' | 'PARCIAL' | 'NAO_ALCANCADO' | null>(null);
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const result = await AiAdaptationService.generateLessonAdaptation(
                profile, lessonTopic, lessonObjective, userId
            );
    ...
    return (
        <div className={`border rounded-xl mb-4 transition-all ${isCompleted ? 'bg-emerald-50 border-emerald-200' : 'bg-white border-indigo-100 shadow-sm'}`}>
            {/* HEADER */}
            <div className="flex items-center justify-between p-3 border-b border-indigo-50/50">
                <div className="flex items-center gap-2">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isCompleted ? 'bg-emerald-100 text-emerald-600' : 'bg-indigo-100 text-indigo-600'}`}>
                        <Accessibility size={16} />
                    </div>
                    <div>
                        <h4 className="text-xs font-black uppercase text-slate-700 tracking-wide">Adaptação: {studentName}</h4>
                        <p className="text-[10px] text-slate-400 font-medium">Seção IX - Adequação Curricular</p>
                    </div>
                </div>
            ...
            {(adaptationGoal || existingAdaptation) ? (
                <div className="p-3 space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            <label className="text-[9px] font-bold text-slate-400 uppercase block mb-1">Objetivo Adaptado</label>
                            <textarea
                        ...
                        <div>
                            <label className="text-[9px] font-bold text-slate-400 uppercase block mb-1">Estratégia / Recurso</label>
                            <textarea
