...
export const CurriculumMatcher: React.FC<CurriculumMatcherProps> = ({ onAddContent }) => {
  ...
  const loadDisciplines = async () => {
    ...
    const { data } = await supabase.from('curriculos_mg').select('disciplina');
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              Conecte habilidades do currículo a questões do ENEM
            </p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4">
        {!selectedDiscipline || !selectedPeriod ? (
          ...
          curriculumItems.map((item) => (
            <div
              key={item.id}
              className={`bg-white rounded-xl border transition-all ${expandedItemId === item.id ? 'border-indigo-500 shadow-md ring-1 ring-indigo-200' : 'border-slate-200 hover:border-indigo-300'}`}
            >
              <div className="p-4">
                ...
                <div className="bg-slate-50 border-t border-slate-100 p-4 rounded-b-xl space-y-3 animate-in slide-in-from-top-2 duration-200">
                  {matchingQuestions[item.id].length === 0 ? (
                    <p className="text-xs text-slate-500 italic">
                      Nenhuma questão relevante encontrada no banco de dados.
                    </p>
                  ) : (
