...
export const SimulationWorkspace: React.FC<SimulationWorkspaceProps> = ({
  ...
  const handleSimAction = async (
    action: 'balance' | 'export_pdf' | 'export_word' | 'generate_ab'
  ) => {
    if (simCart.length === 0) {
    ...
    if (action === 'balance') {
      showToast(
        'info',
        'Para análise de equilíbrio, use o chat pedagógico principal. Esta função será migrada em breve.'
      );
      ...
      try {
        ...
        setTimeout(async () => {
          await withRetry(
            () => exportSimulationToDocx(shuffled, simObservations, 'Versão B', settings),
            { retries: 1 }
          );
        }, 1000);
        ...
        <div className="flex-1 flex flex-col relative">
          ...
          <div className="p-6 bg-white border-b border-slate-200 z-10 shadow-sm">
            <div className="flex gap-4 mb-4">
              <button
                ...
                className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wide transition-all ${simMode === 'manual' ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
              >
                Busca Manual e BNCC
              </button>
              <button
                onClick={() => setSimMode('mirror')}
                className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wide transition-all ${simMode === 'mirror' ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
              >
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder={
                    simMode === 'mirror'
                      ? 'Selecione um plano acima para espelhar...'
                      : 'Digite o tema, habilidade ou ano (ex: Equação 1º Grau)...'
                  }
              ...
              {simMode === 'manual' ? (
                <button
                  ...
                  disabled={simLoading}
                  className="px-6 py-3 bg-indigo-600 text-white rounded-xl font-bold shadow-md hover:bg-indigo-700 disabled:opacity-50"
                >
                ...
                <button
                  ...
                  disabled={simLoading || !selectedTermPlanId}
                  className="px-6 py-3 bg-indigo-600 text-white rounded-xl font-bold shadow-md hover:bg-indigo-700 disabled:opacity-50 flex items-center gap-2"
                >
                  {simLoading ? (
                    <Loader2 className="animate-spin" />
                  ) : (
                    <>
                      <Wand2 size={18} /> Espelhar
                    </>
                  )}
                </button>
              )}
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50">
            <div className="flex items-center justify-between mb-3 text-[11px] text-slate-500">
              <span className="font-black uppercase tracking-[0.2em]">Resultados da busca</span>
            ...
            {simSearchResults.length === 0 ? (
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                {simSearchResults.map((q: any) => {
                  ...
                  const text =
                    q.metadata?.alternativesIntroduction ||
                    q.metadata?.context ||
                  ...
                  return (
                    <div
                      key={q.id}
                      className="bg-white p-4 rounded-xl border border-slate-200 hover:border-indigo-300 transition-all shadow-sm group"
                    >
          ...
          <div className="p-4 bg-white border-t border-slate-200 flex gap-4 items-center">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => handleSimAction('balance')}
                className="px-3 py-2 bg-amber-50 text-amber-700 hover:bg-amber-100 rounded-lg text-[10px] font-black uppercase tracking-wider flex flex-col items-center gap-1 min-w-[4.5rem] transition-colors"
              >
                <ArrowUpDown size={14} /> Equilibrar
              </button>
              <button
                onClick={() => handleSimAction('export_word')}
                className="px-3 py-2 bg-blue-50 text-blue-700 hover:bg-blue-100 rounded-lg text-[10px] font-black uppercase tracking-wider flex flex-col items-center gap-1 min-w-[4.5rem] transition-colors"
              >
                <Download size={14} /> Word
              </button>
              <button
                onClick={() => handleSimAction('generate_ab')}
                className="px-3 py-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 rounded-lg text-[10px] font-black uppercase tracking-wider flex flex-col items-center gap-1 min-w-[4.5rem] transition-colors"
                title="Gerar Prova A e B (Embaralhada)"
              >
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-2">
            {simCart.length === 0 ? (
              ...
              simCart.map((q, idx) => (
                <div
                  key={q.id}
                  className="bg-slate-50 border border-slate-200 p-3 rounded-lg group animate-in slide-in-from-right-2"
                >
                  ...
                  <p className="text-xs text-slate-700 mt-1 line-clamp-2">
                    {q.metadata?.alternativesIntroduction || q.metadata?.context}
                  </p>
                </div>
              ))
            )}
        ...
        >
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col relative z-10 motion-safe:animate-in motion-safe:zoom-in-95 duration-200 overflow-hidden">
                ...
                {previewQuestion.metadata?.alternatives?.map((alt) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 pt-0.5">{alt.text}</div>
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3 z-20">
              ...
              <button
                onClick={() => {
                  handleAddToCart(previewQuestion);
                  setPreviewQuestion(null);
                }}
                className="px-4 py-2 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow-sm hover:shadow hover:-translate-y-0.5 transition-all flex items-center gap-2"
              >
