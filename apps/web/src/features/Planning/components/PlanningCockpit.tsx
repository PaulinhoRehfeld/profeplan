...
export const PlanningCockpit: React.FC<PlanningCockpitProps> = ({
  ...
  // 🔄 ITERATIVE FEEDBACK HANDLERS
  const handleRegeneratePlan = async (feedback: string) => {
    setIsRegenerating(true);
    try {
      // Criar prompt de regeneração com feedback
      const regeneratePrompt = `[FEEDBACK DO PROFESSOR - PRIORIDADE CRÍTICA]
O professor solicitou o seguinte ajuste no plano anterior:
"${feedback}"

REGENERE o plano incorporando esta mudança imediatamente. Ignore qualquer regra anterior que conflite com este feedback.`;

      // Disparar regeneração
      const syntheticEvent = { preventDefault: () => {} } as React.FormEvent;
      await handleSendMessage(syntheticEvent, regeneratePrompt);

      // Atualizar último conteúdo gerado (será usado na confirmação)
      if (messages.length > 0) {
        const lastAssistantMsg = messages.filter((m) => m.role === MessageRole.ASSISTANT).pop();
        if (lastAssistantMsg) {
          setLastGeneratedContent(lastAssistantMsg.content);
        }
      }
    } catch (error) {
  ...
  const handleSaveFeedbackAsDefault = async (feedback: string) => {
    try {
      await saveFeedbackPreference(userId, feedback);
      console.log('✅ Feedback salvo como preferência padrão!');
    } catch (error) {
      console.error('Erro ao salvar feedback como padrão:', error);
    }
  };
  ...
  const [materialType, setMaterialType] = useState<'resumo' | 'teorico' | 'exercicios' | null>(
    null
  );
...
- Nível de Dificuldade: ${difficulty}`;
  ...
  return (
    <div className="flex flex-col lg:flex-row bg-slate-50 h-full relative overflow-hidden">
          ...
          <div className="flex justify-between items-center px-1">
            ...
            <span className="bg-emerald-100 text-emerald-700 text-[10px] font-bold px-2 py-0.5 rounded-full">
              {parsedLessons.length > 0
                ? `${Object.keys(lessonTracking).length}/${parsedLessons.length} Geradas`
                : '0/0'}
            </span>
          </div>
        </div>
          ...
          {!selectedTermPlanId ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-400 p-8 text-center opacity-60">
              ...
              <p className="text-xs font-medium">
                Selecione um planejamento acima para ver as aulas.
              </p>
            </div>
          ) : parsedLessons.length === 0 ? (
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className={`font-bold leading-snug truncate ${isPrepared && selectedLesson?.number !== lesson.number ? 'line-through text-slate-400 decoration-slate-300 decoration-2' : ''}`}
                      >
                        {lesson.title}
                      </p>
                      {selectedLesson?.number === lesson.number && (
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 custom-scrollbar space-y-6">
          {messages.length === 0 ? (
            ...
            messages.map((msg) => (
              ...
              >
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-sm ${msg.role === MessageRole.USER ? 'bg-indigo-600 text-white' : 'bg-white text-emerald-600 border border-emerald-100'}`}
                >
                  {msg.role === MessageRole.USER ? <User size={16} /> : <Bot size={16} />}
                </div>
                <div className={`max-w-[85%] flex flex-col items-start`}>
                  <div
                    className={`p-5 rounded-2xl shadow-sm text-sm leading-relaxed whitespace-pre-wrap ${msg.role === MessageRole.USER ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-white border border-slate-100 text-slate-700 rounded-tl-none'}`}
                  >
                    {msg.content}
                  </div>
                  {msg.role === MessageRole.ASSISTANT && (
                    <div className="flex items-center gap-2 mt-3 w-full flex-wrap">
                      <button
                      ...
                      <button
                        ...
                        className="px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest border border-slate-300 hover:bg-slate-50 disabled:opacity-50"
                      >
                        Word
                      </button>
                      {/* 🔄 BOTÃO DE FEEDBACK ITERATIVO  */}
                      <button
                        onClick={() => {
                          setLastGeneratedContent(msg.content);
                          setShowFeedbackWidget(true);
                        }}
                        className="flex items-center gap-2 px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest bg-purple-600 text-white hover:bg-purple-700 transition-all"
                      >
        ...
        >
          <div className="w-full max-w-3xl bg-white rounded-[2rem] shadow-2xl border border-slate-100 flex flex-col max-h-[90vh]">
            ...
            <div className="px-6 py-4 overflow-y-auto custom-scrollbar text-sm text-slate-700">
              <pre className="whitespace-pre-wrap font-sans text-sm">{latestAssistantMessage}</pre>
            </div>
          </div>
        </div>
      )}
      ...
      {/* 🔄 ITERATIVE FEEDBACK WIDGET */}
      <IterativeFeedbackWidget
        isVisible={showFeedbackWidget}
        onClose={() => setShowFeedbackWidget(false)}
        onRegeneratePlan={handleRegeneratePlan}
        onSaveFeedbackAsDefault={handleSaveFeedbackAsDefault}
        isRegenerating={isRegenerating}
        latestGeneration={lastGeneratedContent}
      />
    </div>
  );
};
