...
import {
  getClasses as getUserClasses,
  getClassDetails as getSupabaseClassDetails,
...
import { getLocalClassesForUser, getLocalClassDetails } from '../../services/localStorageService';
import { useGlobalPlanning } from '../../contexts/GlobalPlanningContext';
import {
  createSchoolStudent,
  findSchoolStudentBySchoolAndName,
...
export const usePDIManager = (userId: string, userProfile: UserProfile) => {
  ...
  const [lastAdaptationDetails, setLastAdaptationDetails] = useState<{
    id: string;
    studentName: string;
    lessonTopic: string;
  } | null>(null);
        ...
        if (source === 'planning' && (lessonTitle || lessonNumber)) {
          const targetClass =
            loadedClasses.find((cls: any) => {
              ...
              return (
                (!subject || subj.includes(subject.toLowerCase())) &&
  ...
  const loadInitialData = async (): Promise<Class[]> => {
    ...
    try {
      ...
      const mappedLessons = genContents
        ? genContents.map((item: any) => ({
            id: item.id,
            topic: item.title,
        ...
        return {
          ...
          students: Array.isArray(c.students)
            ? c.students.map((s: any) => ({
                id: s.id,
                name:
                  typeof s.name === 'object' ? s.name?.name || 'Sem Nome' : s.name || 'Sem Nome',
                class_id: c.id,
    ...
    if (localData) {
      const mappedClass: Class = {
        ...
        students: localData.students.map((s: any) => ({
          ...
          current_school_id:
            (userProfile as any)?.active_school_id ?? userProfile?.school_id ?? null,
      ...
      const needs =
        mappedClass.students?.filter((s: Student) => {
          ...
          const hasObs =
            typeof s.pedagogical_observations === 'string' &&
            s.pedagogical_observations.trim().length > 0;
          const hasLegacyObs =
            typeof s.observations === 'string' && s.observations.trim().length > 0;
  ...
  const handleGenerateAdaptation = async (student: Student) => {
    ...
    try {
      ...
      const gradeLevel =
        selectedClass?.grade ??
        (selectedClass?.year !== undefined && selectedClass?.year !== null
          ? String(selectedClass.year)
          : 'Geral');
      ...
      if (studentError)
        console.warn('Could not fetch detailed PDI data, using basics.', studentError);
      ...
      const studentContext = {
        ...
        diagnostico_clinico:
          pdiData.clinical_health?.diagnosis_cid ||
          student.deficiencies?.join(', ') ||
          'Não informado',
      ...
      const evaluationMatch = resultJSON.adaptacao_metodologica?.match(
        /Avalia[cç][aã]o[^:]*:\s*([\s\S]+)/i
      );
      const evaluationDifferentiated =
        evaluationMatch?.[1]?.trim()?.slice(0, 1200) || 'Não especificado na geração atual.';
  ...
  const handleValidate = async (studentId: string, content: string) => {
    ...
    const finalContent =
      sanitizedContent.length > 0 ? sanitizedContent : content.substring(0, 5000);
    ...
    try {
      let schoolIdForSchoolStudent =
        ...
        (student
          ? ((student as any).current_school_id ?? (student as any).school_id ?? null)
          : null) ??
        (userProfile as any)?.active_school_id ??
        userProfile?.school_id ??
        (userProfile as any)?.school?.id ??
      ...
      if (!schoolIdForSchoolStudent) {
        try {
          ...
          try {
            ...
            if (!tsErr && (tsRow as any)?.school_id) {
              ...
              console.log(
                '[PDI] Resolved schoolIdForSchoolStudent via teacher_schools:',
              ...
              try {
                ...
                if (!tsLatestErr && (tsLatest as any)?.school_id) {
                  ...
                  console.log(
                    '[PDI] Resolved schoolIdForSchoolStudent via teacher_schools (latest row):',
                    schoolIdForSchoolStudent
                  );
                } else if (tsLatestErr) {
        ...
        try {
          ...
          if (!rpcErr && rpcData) {
            ...
            const candidate =
              (rpcData as any)?.school_id ?? (rpcData as any)?.id ?? (rpcData as any);
      ...
      if (!schoolStudentId) {
        schoolStudentId = await (async () => {
          ...
          const found = await findSchoolStudentBySchoolAndName(
            schoolIdForSchoolStudent,
            student.name
          );
      ...
      if (schoolStudentId) {
        try {
          ...
          if (payload) {
            ...
            console.warn(
              '[PDI] Missing block9Payload in local state; skipping block_9_content persist.'
            );
          }
        } catch (e) {
    ...
    try {
      ...
      if (!saved) {
        ...
        setError(
          'Adaptação validada, mas não foi possível salvar o documento automaticamente. Você pode exportar manualmente.'
        );
      }
    } catch (e) {
  ...
  const handleDownloadStudentDoc = async (student: Student, content: string) => {
    ...
    try {
      ...
      await exportToDocx(markdownContent, `ADAPTACAO_${student.name.replace(/[^a-z0-9]/gi, '_')}`, {
  ...
  const handleExportDoc = async () => {
    if (!selectedLesson) return;
    const validAdaptations = (Object.values(adaptations) as StudentAdaptation[]).filter(
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      if (!adaptationRecords || adaptationRecords.length === 0) {
        setError(
          `Sem histórico de adaptações para o aluno ${student.name}. Valide pelo menos uma adaptação.`
        );
        return;
      }
  ...
  const handleSaveFeedback = async (feedbackData: any) => {
    ...
    try {
      ...
      const { data: record } = await supabase
        .from('pdi_records')
        .select('content')
        .eq('id', lastAdaptationDetails.id)
        .single();
      if (record) {
        const newContent = {
          ...record.content,
  ...
  // Viewing State
  const [viewingAdaptation, setViewingAdaptation] = useState<{
    studentName: string;
    content: string;
  } | null>(null);
