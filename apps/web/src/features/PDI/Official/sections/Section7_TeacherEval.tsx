  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between border-b border-slate-100 pb-4">
        ...
        <button
          onClick={() =>
            append({ bimester: '1', subject: '', autonomy_level: 'MUITO_SUPORTE', diagnosis: '' })
          }
      ...
      {fields.length === 0 ? (
        <div className="text-center py-12 bg-slate-50 rounded-xl border border-dashed border-slate-200">
          ...
          <p className="text-xs text-slate-400 mt-1">
            Clique em "Adicionar Avaliação" para começar.
          </p>
        </div>
      ) : (
        <div className="space-y-6">
          {fields.map((field, index) => (
            <div
              key={field.id}
              className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm relative group hover:border-emerald-200 transition-all"
            >
              <button
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider">
                  Parecer Descritivo / Diagnóstico
                </label>
                <textarea
