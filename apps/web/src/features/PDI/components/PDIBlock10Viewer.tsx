  ...
  const loadEntries = async () => {
    ...
    try {
      ...
      if (pdi && pdi.block_10_entries) {
        ...
        // Sort by date desc
        const sorted = [...mappedEntries].sort(
  ...
  if (entries.length === 0) {
    return (
      <div className="bg-slate-50 border-2 border-dashed border-slate-300 rounded-2xl p-12 text-center">
        ...
        <h3 className="text-xl font-bold text-slate-900 mb-2">Nenhuma Avaliação Registrada</h3>
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {entries.map((entry) => {
          ...
          const percentual = (
            (entry.professor_nota_alcancada / entry.professor_valor) *
            100
          ).toFixed(1);
                  ...
                  <div className="pt-4 border-t border-slate-100">
                    <div className="flex items-center gap-4 text-xs text-slate-500">
                      ...
                      {entry.ia_generated_at && (
                        <span>
                          IA gerada em: {new Date(entry.ia_generated_at).toLocaleString('pt-BR')}
                        </span>
                      )}
                    </div>
                  </div>
