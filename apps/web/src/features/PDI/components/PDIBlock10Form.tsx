...
const PDIBlock10Form: React.FC<PDIBlock10FormProps> = ({ pdiId, studentName, userId }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { data: evaluation, error: saveError } = await PdiDocumentService.addBlock10Evaluation(
        pdiId,
        {
      ...
      await PdiDocumentService.updateBlock10WithAI(pdiId, evaluation.id, iaDiagnostico);
      ...
      <div className="space-y-6">
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <span
              className={`text-2xl font-black ${
                parseFloat(percentual) >= 70
                  ? 'text-green-600'
                  : parseFloat(percentual) >= 50
                    ? 'text-yellow-600'
                    : 'text-red-600'
              }`}
            >
        ...
        <div>
            ...
            {[
              ...
              <button
                ...
                className={`px-4 py-3 rounded-xl font-bold text-sm transition-all ${
                  professorAutonomia === option.value
          ...
          <p className="text-xs text-slate-500 mt-2">
            <strong>Total:</strong> Realizou sozinho | <strong>Parcial:</strong> Precisou de algum
            apoio | <strong>Dependente:</strong> Precisou de apoio constante
          </p>
        </div>
