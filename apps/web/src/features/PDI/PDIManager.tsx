          ...
          <div className="flex items-center gap-4 mt-2">
            ...
            {selectedClass && (
              <div className="relative">
                ...
                <ChevronDown
                  size={14}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-indigo-500 pointer-events-none"
                />
              </div>
            )}
          </div>
              ...
              {studentsWithNeeds.length === 0 ? (
                <div className="p-10 bg-white rounded-2xl border border-slate-200 text-center">
                  ...
                  <p className="mt-2 text-xs text-slate-400 max-w-md mx-auto">
                    Para que os alunos apareçam aqui, marque-os em <strong>Minhas Turmas</strong>{' '}
                    com necessidade de adaptação (campo de observações e/or switch de inclusão).
                  </p>
                  <p className="mt-4 text-xs text-slate-400 max-w-md mx-auto italic">
                    Para cadastrar um aluno de inclusão, acesse o menu{' '}
                    <strong>Minhas Turmas</strong>, edite o perfil do aluno e ative a sinalização de
                    inclusão.
                  </p>
                </div>
              ) : (
                <div className="space-y-6">
                  {studentsWithNeeds.map((student) => (
                    <div
                      key={student.id}
                      className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col md:flex-row gap-6 transition-all hover:shadow-md"
                    >
                      {/* Student Info */}
                          ...
                          <div>
                            ...
                            <div className="flex gap-2 mt-1">
                              {student.deficiencies?.map((def, i) => (
                                <span
                                  key={i}
                                  className="px-2 py-0.5 bg-rose-50 text-rose-600 text-[10px] font-bold uppercase rounded-md border border-rose-100"
                                >
                                  {def}
                                </span>
                              ))}
