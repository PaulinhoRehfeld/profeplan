...
class SimulationAnalyticsService {
  ...
  async trackSearch(event: SearchAnalyticsEvent): Promise<void> {
    try {
      const { error } = await supabase.from(this.SEARCH_EVENTS_TABLE).insert([event]);
  ...
  async trackQuestionView(event: QuestionViewEvent): Promise<void> {
    try {
      const { error } = await supabase.from(this.VIEW_EVENTS_TABLE).insert([event]);
  ...
  async getSummary(userId?: string, days: number = 30): Promise<AnalyticsSummary> {
    try {
      ...
      let viewQuery = supabase.from(this.VIEW_EVENTS_TABLE).select('*').gte('timestamp', sinceISO);
