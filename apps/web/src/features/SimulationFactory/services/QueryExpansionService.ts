...
class QueryExpansionService {
   ...
   */
  async searchExpanded(query: string, limit: number = 50): Promise<QuestionDatabaseRow[]> {
    try {
      ...
      let supabaseQuery = supabase.from('enem_questions').select('id, content, metadata');

      // Aplicar OR filters
      const orFilters = expandedTerms.map((term) => `content.ilike.%${term}%`).join(',');

      const { data, error } = await supabaseQuery.or(orFilters).limit(limit);
