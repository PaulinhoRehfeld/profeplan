...
class QuestionBankService {
    ...
    async search(params: QuestionSearchParams): Promise<QuestionSearchResult> {
        ...
        console.log(`[QuestionBank] 🔍 Search: "${query}" [Areas: ${areas.join(', ') || 'All'}] [Limit: ${limit}]`);
        ...
        try {
            ...
            console.log(`[QuestionBank] ✅ Final result: ${finalQuestions.length} questions (${searchSource})`);
     ...
     * Busca rápida
     */
    async quickSearch(query: string, limit: number = 5): Promise<Array<{ id: number; preview: string }>> {
        try {
            const result = await this.search({ query, limit });

            return result.questions.map(q => ({
                id: q.id,
                preview: q.metadata?.alternativesIntroduction?.substring(0, 100) ||
     ...
     * Track analytics
     */
    private trackSearch(query: string, areas: string[], cacheHit: boolean, resultCount: number): void {
