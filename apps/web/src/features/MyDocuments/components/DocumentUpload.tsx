...
export const DocumentUpload: React.FC<DocumentUploadProps> = ({
  ...
  const [category, setCategory] = useState<'course_plan' | 'book' | 'didactic_material'>(
    'course_plan'
  );
  ...
  const startIngestion = async () => {
    ...
    try {
      ...
      if (!rawText || rawText.trim().length === 0) {
        throw new Error(
          'Não foi possível extrair nenhum texto legível deste PDF. Certifique-se de que o arquivo não está corrompido ou protegido por senha.'
        );
      }
      ...
      setStep('parsing');
      setProgressText(
        'Processando com a Inteligência Artificial: Estruturando Markdown, extraindo metadados e gerando relatório de curadoria...'
      );
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 p-8 rounded-[2rem] max-w-xl mx-auto text-slate-100 shadow-2xl backdrop-blur-xl">
      <h3 className="text-lg font-black uppercase tracking-wider text-blue-400 mb-6">
