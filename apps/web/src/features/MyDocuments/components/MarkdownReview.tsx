...
export const MarkdownReview: React.FC<MarkdownReviewProps> = ({
  documentId,
  userId,
  onApproved,
  onReprocess,
  ...
  const handleReprocess = async () => {
    const confirm = window.confirm(
      'Deseja realmente reprocessar este documento? O registro atual será removido para re-envio.'
    );
    if (confirm) {
      try {
        await supabase.from('teacher_documents').delete().eq('id', documentId);
        onReprocess();
      } catch (e) {
        console.error(e);
      }
        ...
        <div className="flex gap-4 p-5 bg-amber-950/20 border border-amber-900/30 rounded-2xl">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[11px] text-amber-500/80 leading-relaxed">
              A extração por IA identificou que algumas partes do PDF original (tabelas, caracteres
              especiais ou formatação) podem ter sido comprometidas. Recomendamos revisar e ajustar
              o Markdown abaixo antes de aprovar.
            </p>
          </div>
        </div>
      ) : (
        <div className="flex gap-4 p-5 bg-emerald-950/10 border border-emerald-900/20 rounded-2xl">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[11px] text-emerald-500/80 leading-relaxed">
              Os algoritmos de parser confirmaram alta fidelidade de extração em relação ao PDF
              original.
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4 text-xs">
            ...
            <div>
              ...
              <span className="text-slate-300">
                {metadata.workload ? `${metadata.workload} horas` : 'N/A'}
              </span>
            </div>
          </div>
            ...
            <div className="text-xs space-y-1">
              ...
              <div className="flex flex-wrap gap-1">
                {metadata.skills.map((s: string) => (
                  <span
                    key={s}
                    className="px-2 py-0.5 bg-slate-800 text-blue-300 rounded text-[10px] font-bold"
                  >
                    {s}
                  </span>
                ))}
