...
export const MyDocumentsManager: React.FC<MyDocumentsManagerProps> = ({ userId }) => {
  ...
  const [compareDocs, setCompareDocs] = useState<{ doc1Id: string; doc2Id: string }>({
    doc1Id: '',
    doc2Id: '',
  });
  ...
  const handleDelete = async (docId: string) => {
    const confirm = window.confirm(
      'Deseja realmente excluir este documento? Todos os chunks e dados de RAG correspondentes serão removidos.'
    );
    if (confirm) {
        ...
        <div className="text-center p-12 bg-slate-900 border border-slate-800 rounded-3xl max-w-lg mx-auto space-y-6">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-400 leading-relaxed">
              Envie Planos de Curso, Livros Didáticos ou Materiais Complementares para que os
              agentes de inteligência respondam alinhados à sua grade de aulas.
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 gap-4">
          {Object.entries(groupedDocs).map(([filename, versions]: [string, any[]]) => {
            ...
            return (
              <div
                key={filename}
                className="bg-slate-900/60 border border-slate-850 hover:border-slate-800 p-6 rounded-3xl flex flex-col md:flex-row md:items-center justify-between gap-4 transition-all"
              >
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-slate-500">
                      Versão mais recente: V{latestDoc.version}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-4 text-xs text-slate-400">
                    ...
                    {latestDoc.extraction_score && (
                      <>
                        ...
                        <span
                          className={
                            latestDoc.extraction_score >= 80 ? 'text-emerald-400' : 'text-amber-400'
                          }
                        >
