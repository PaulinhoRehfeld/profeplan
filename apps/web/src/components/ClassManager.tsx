...
const ClassManager: React.FC<{ userId: string; userProfile?: any }> = ({ userId, userProfile }) => {
  ...
  const handleSaveNewClass = async (data: {
    name: string;
    subject: string;
    students: string[];
  }) => {
    ...
    try {
      ...
      console.warn(
        '[ClassManager] Supabase save failed on createClass. Saved locally.',
        err?.message
      );
      if (!is401) throw err;
    }
  ...
  const handleImportComplete = async (parsedData: {
    ...
    try {
      ...
      const is401 =
        ...
        err?.message?.includes('Sessão expirada');
      console.warn(
        '[ClassManager] Supabase save failed (session issue?). Saved locally.',
        err?.message
      );
      if (is401) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">
            Gerencie suas salas de aula e listas de alunos
          </p>
        </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center h-64 text-slate-300">
          ...
          <p className="font-black uppercase tracking-widest text-[10px]">
            Consultando Turmas Cadastradas...
          </p>
        </div>
        ...
        <div className="bg-slate-50 border-2 border-dashed border-slate-100 rounded-[2.5rem] p-20 flex flex-col items-center text-center">
          ...
          <h3 className="text-lg font-black text-slate-900 uppercase italic tracking-tight mb-2">
            Sua Escola Digital está Vazia
          </h3>
