...
const ClassBatchImportModal: React.FC<ClassBatchImportModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  schoolId,
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setModalStudents(
        parsed.students.map((s: any) => ({
          name: typeof s === 'object' ? s.name || 'Sem Nome' : s,
          needsPdi: false,
        }))
      );
      setStep('review');
            ...
            <div className="flex flex-col items-center justify-center h-full space-y-8 py-10">
              ...
              <div className="text-center max-w-md">
                ...
                <p className="text-sm text-slate-500">
                  Envie um PDF com a lista de alunos (chamada) ou um arquivo de texto.
                </p>
              </div>
              ...
              {error && (
                <p className="text-red-500 font-bold text-xs bg-red-50 px-4 py-2 rounded-lg">
                  {error}
                </p>
              )}
            </div>
                ...
                <div className="border border-slate-100 rounded-2xl overflow-hidden max-h-[300px] overflow-y-auto bg-slate-50/50">
                  <table className="w-full text-left">
                    <thead className="bg-slate-100 border-b border-slate-200">
                      <tr>
                        <th className="px-6 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest w-16">
                          #
                        </th>
