...
const SchoolStudentSelector: React.FC<SchoolStudentSelectorProps> = ({ onImport, onCancel }) => {
  ...
  const handleImport = () => {
    const selectedNames = students.filter((s) => selectedIds.has(s.id)).map((s) => s.name);
    ...
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center h-full">
        ...
        <p className="text-slate-600 font-medium mb-4">
          Você precisa estar vinculado a uma escola para importar alunos.
        </p>
        <button
          onClick={onCancel}
            ...
            return (
              ...
              >
                ...
                <div className="text-left">
                  <h2
                    className={`text-sm font-bold ${isSelected ? 'text-blue-900' : 'text-slate-700'}`}
                  >
                    {student.name}
                  </h2>
                  {student.birth_date && (
