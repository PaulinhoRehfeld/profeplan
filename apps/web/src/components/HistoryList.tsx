...
const HistoryList: React.FC<{ userId: string; onSelectLesson: (content: string) => void }> = ({
  userId,
  onSelectLesson,
}) => {
  ...
  const getTypeLabel = (type: string) => {
    switch (type.toLowerCase()) {
      ...
      case 'trimestral':
        return 'Planej. Trimestral';
      case 'aula':
        return 'Atividade / Aula';
      case 'avaliacao':
        return 'Avaliação';
      default:
        return 'Documento';
      ...
      {filteredItems.length === 0 ? (
        <div className="bg-slate-50 border-2 border-dashed border-slate-100 rounded-[2.5rem] p-20 flex flex-col items-center text-center">
          ...
          <p className="text-[10px] text-slate-300 font-bold uppercase mt-2">
            Gere planos, aulas ou slides para preencher sua memória.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredItems.map((item) => (
            ...
            >
              <div
                className={`absolute top-0 right-0 w-24 h-24 blur-3xl rounded-full -mr-12 -mt-12 transition-colors ${
                  item.type === 'presentation'
                    ? 'bg-purple-50/50 group-hover:bg-purple-100'
                    : 'bg-blue-50/50 group-hover:bg-blue-100'
                }`}
              ></div>
              ...
              <div className="flex items-start justify-between mb-4 relative z-10">
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg ${
                    item.type === 'presentation' ? 'bg-purple-600' : 'bg-slate-900'
                  }`}
                >
              ...
              <div className="mb-2">
                <span
                  className={`text-[9px] font-black uppercase tracking-widest py-1 px-2 rounded-lg ${
                    item.type === 'presentation'
              ...
              <div className="flex gap-2 relative z-10 mt-auto">
                <button
                  ...
                  className={`flex-1 py-4 rounded-xl font-black text-xs uppercase tracking-widest transition-colors flex items-center justify-center gap-2 ${
                    item.type === 'presentation'
                ...
                {item.canva_json && (
                  <button
                    ...
                    onClick={() =>
                      alert('Para usar os dados no Canva, abra a apresentação no modo Estúdio!')
                    }
                  >
                    <Download size={14} />
                  </button>
