                                    ...
                                    <>
                                        ...
                                        <div className="space-y-2.5">
                                            {meta.alternatives?.map((alt: any, idx: number) => (
                                                <div
                                                    key={idx}
                                                    className={`flex gap-3 p-3 rounded-lg border text-sm transition-colors ${alt.isCorrect
                                                        ? 'bg-green-50 border-green-200 ring-1 ring-green-100'
                                                        : 'bg-white border-gray-200 hover:bg-gray-50'
                                                        }`}
                                                >
                                                    <span className={`font-bold w-6 shrink-0 ${alt.isCorrect ? 'text-green-700' : 'text-gray-400'}`}>
                                                        {alt.letter})
                                                    </span>

                                                    <div className="flex-1">
                                                        {alt.text && <span className={alt.isCorrect ? 'text-green-900' : 'text-gray-700'}>{alt.text}</span>}
                                                        {alt.file && (
                                                            <img src={alt.file} alt={`Alternativa ${alt.letter}`} className="mt-2 h-24 border rounded bg-white p-1" />
                                                        )}
                                                    </div>

                                                    {alt.isCorrect && <CheckCircle className="h-5 w-5 text-green-600 shrink-0" />}
                                                </div>
                                            ))}
                                        </div>
                                    </>
