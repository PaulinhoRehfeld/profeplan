...
export const StudentPDIProfile: React.FC<StudentPDIProfileProps> = ({ studentId, onClose }) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<PDIProfileData & { deficiencies: string[] }>({
    ...
    const loadStudentData = async () => {
      ...
      try {
        ...
        if (!finalSSId && student.current_school_id) {
          ...
          const existingSS = await findSchoolStudentBySchoolAndName(
        ...
        if (finalSSId) {
          ...
          if (ssData?.pdi_data) {
            ...
            setValue('student_data.name', student.name);
            const deficiencies = (ssData.pdi_data as PDIProfileData & { deficiencies?: string[] })
              ?.deficiencies;
            if (deficiencies) {
        ...
        const dob =
          (student as any).dob || (student as any).birth_date || (student as any).data_nascimento;
        if (dob) {
  ...
  const onSubmit = async (formData: PDIProfileData & { deficiencies: string[] }) => {
    ...
    if (!schoolStudentId) {
      alert(
        'Erro: ID de vínculo escolar (school_student_id) não encontrado. Não é possível salvar o PDI.'
      );
      setLoading(false);
      return;
    }
      ...
      <div className="bg-white border-b border-indigo-100 p-6 flex justify-between items-center shadow-sm z-10">
        <div>
          ...
          <p className="text-xs text-slate-500 font-bold uppercase tracking-wider mt-1">
            Preenchimento Oficial • Modelo SEE/MG
          </p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={onClose}
            className="px-4 py-2 text-sm text-slate-400 font-bold hover:text-slate-600"
          >
            Fechar
          </button>
          <button
          ...
          <div className="max-w-3xl mx-auto animate-in slide-in-from-left-4 fade-in duration-300 space-y-8">
            <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl flex gap-3 text-blue-700">
              ...
              <div>
                ...
                <p className="text-xs mt-1">
                  Estes dados são identificados automaticamente do cadastro do aluno.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100">
              ...
              <p className="text-[10px] text-slate-400">
                Selecione as tags para identificação rápida na lista.
              </p>
            </div>
                ...
                <div className="space-y-4 col-span-2">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                      ...
                      {
                        key: 'attention_sustained',
                        label: 'Atenção Sustentada',
                        section: 'cognitive',
                      },
                      {
                        key: 'memory_short_term',
                        label: 'Memória Curto Prazo',
                        section: 'cognitive',
                      },
                      {
                        key: 'orders_simple',
                        label: 'Compreensão Ordens Simples',
                        section: 'cognitive',
                      },
                      {
                        key: 'interaction_intent',
                        label: 'Intenção Comunicativa',
                        section: 'communication',
                      },
                    ].map((item) => (
                      <label
                        key={item.key}
                        className="block bg-slate-50 p-3 rounded-xl border border-slate-100"
                      >
