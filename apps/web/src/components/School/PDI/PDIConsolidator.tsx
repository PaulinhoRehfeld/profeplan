...
import { generateFinalPDIReport } from '../../../services/ai/AiPdiService';
...
export const PDIConsolidator: React.FC<PDIConsolidatorProps> = ({
  studentId,
  studentName,
  onClose,
}) => {
  ...
  if (loading && !profileData)
    return <div className="p-10 text-center">Carregando dados consolidados...</div>;
    ...
    <div className="bg-white min-h-[600px] flex flex-col h-full rounded-2xl shadow-xl overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 p-6 text-white flex justify-between items-center">
        <div>
          ...
          <p className="text-emerald-200/60 text-xs font-bold uppercase tracking-widest mt-1">
            Item XI - Relatório Final (IA Generator)
          </p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto bg-slate-50 p-8 flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-1/3 space-y-6">
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
            <h3 className="text-xs font-black uppercase text-slate-400 tracking-wider mb-4 border-b border-slate-100 pb-2">
              Fontes de Dados
            </h3>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                <div className="flex items-center gap-3">
                  <div
                    className={`p-2 rounded-lg ${profileData?.psychomotor ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'}`}
                  >
                    <BookOpen size={16} />
                  </div>
                  <div>
                    ...
                    <p className="text-xs text-slate-400">
                      {profileData?.psychomotor ? 'Preenchido' : 'Pendente'}
                    </p>
                  </div>
                </div>
                {profileData?.psychomotor && (
              ...
              <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                <div className="flex items-center gap-3">
                  <div
                    className={`p-2 rounded-lg ${evaluations.length > 0 ? 'bg-blue-100 text-blue-600' : 'bg-orange-100 text-orange-600'}`}
                  >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">
                      {evaluations.length} Professores registraram
                    </p>
                  </div>
                </div>
          ...
          <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl text-blue-800 text-xs leading-relaxed">
            <p className="font-bold mb-2 flex items-center gap-2">
              <BrainCircuit size={14} /> Como funciona a IA?
            </p>
            O Gerador analisa o Perfil Clínico + Avaliações dos Professores e redige um texto
            técnico coeso para o campo "Relatório Final", pronto para assinatura.
          </div>
        </div>
