...
export const StudentManagement: React.FC<StudentManagementProps> = ({
  ...
  const togglePDI = (need: string) => {
    setFormData((prev) => {
      ...
      return {
        ...prev,
        pdi_needs: exists ? prev.pdi_needs.filter((n) => n !== need) : [...prev.pdi_needs, need],
      };
    });
  };
                    ...
                    {student.pdi_needs && student.pdi_needs.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {student.pdi_needs.slice(0, 3).map((pdi) => (
                          <span
                            key={pdi}
                            className="px-2 py-0.5 bg-amber-100 text-amber-800 text-[10px] font-bold rounded-full border border-amber-200"
                          >
                            {pdi}
                          </span>
                        ))}
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {PDI_OPTIONS.map((option) => (
                    <button
                      ...
                      className={`text-left px-3 py-2 rounded-lg text-xs font-bold transition border ${
                        formData.pdi_needs.includes(option)
        ...
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-slate-600 mb-6">
              ...
              <br />
              Esta ação moverá o aluno para o arquivo morto.
            </p>
      ...
      {pdiStudentId && (
        <div className="fixed inset-0 z-[100] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200">
            <StudentPDIProfile studentId={pdiStudentId} onClose={() => setPdiStudentId(null)} />
          </div>
        </div>
      )}
    </div>
