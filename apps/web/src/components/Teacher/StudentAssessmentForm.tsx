...
import {
  ...
  pdiTeacherEvaluationForm,
  AutonomyLevel,
  ComprehensionLevel,
} from '../../types/pdi-schema';
...
export const StudentAssessmentForm: React.FC<StudentAssessmentFormProps> = ({
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<pdiTeacherEvaluationForm>({
  ...
  const onSubmit = async (formData: pdiTeacherEvaluationForm) => {
    ...
    try {
      ...
      // 2. Save Evaluation
      const { error: evalError } = await (supabase as any).from('pdi_teacher_evaluations').upsert(
        {
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div>
          ...
          />
          {errors.pedagogical_diagnosis && (
            <span className="text-red-500 text-xs">{errors.pedagogical_diagnosis.message}</span>
          )}
        </div>
