    ...
    <div className="bg-blue-600 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-300">
      ...
      {step === 'uploading' || step === 'parsing' ? (
        <div className="flex flex-col items-center text-center py-10">
          ...
          <p className="text-blue-200 text-[10px] font-bold uppercase tracking-[0.3em] mt-3">
            Extraindo inteligência pedagógica
          </p>
        </div>
      ) : (
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-blue-100 text-sm font-bold mt-1">
                Encontramos{' '}
                <span className="text-white font-black">{tempData?.students.length} alunos</span> na
                turma <span className="text-white font-black">{tempData?.className}</span> (
                {tempData?.subject}).
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {tempData?.students.map((student, i) => {
                ...
                return (
                  <div
                    key={i}
                    className="text-[10px] font-bold uppercase tracking-tight text-blue-100 flex items-center gap-2"
                  >
      ...
      {error && (
        <div className="mt-4 p-4 bg-red-500/20 text-white rounded-xl text-center font-bold text-xs border border-white/10">
          {error}
          <button onClick={onCancel} className="block w-full mt-2 text-[10px] underline">
            Tentar novamente
          </button>
        </div>
      )}
