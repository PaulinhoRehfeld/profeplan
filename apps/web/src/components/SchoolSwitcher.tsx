  ...
  useEffect(() => {
    const loadSchools = async () => {
      ...
      try {
        ...
        const schoolList =
          links?.flatMap((link) => (link.schools ? [link.schools as unknown as School] : [])) || [];
            ...
            {schools.map((school) => (
              <button
                ...
                className={`w-full px-4 py-3 text-left hover:bg-slate-50 transition-colors flex items-center justify-between group ${
                  school.id === activeSchool?.id ? 'bg-indigo-50' : ''
                }`}
              >
                <div className="flex-1">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">INEP: {school.inep_code}</p>
                </div>
