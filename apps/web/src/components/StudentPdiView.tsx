...
const StudentPdiView: React.FC<StudentPdiViewProps> = ({ studentId }) => {
    ...
    const loadData = async () => {
        ...
        try {
            // Fetch Name
            const { data: student } = await supabase.from('school_students').select('name').eq('id', studentId).single();
    ...
    const getIcon = (type: string) => {
        switch (type) {
            case 'EVALUATION': return <Activity size={16} className="text-purple-600" />;
            case 'LESSON_PLAN': return <BookOpen size={16} className="text-blue-600" />;
            case 'OCCURRENCE': return <AlertCircle size={16} className="text-red-600" />;
            case 'OBSERVATION': return <Quote size={16} className="text-emerald-600" />;
    ...
    const getTitle = (type: string) => {
        switch (type) {
            case 'EVALUATION': return 'Avaliação da Aprendizagem (Bloco X)';
            case 'LESSON_PLAN': return 'Proposta Pedagógica (Bloco VIII)';
            case 'OCCURRENCE': return 'Registro Ocorrência (Bloco VII)';
            case 'OBSERVATION': return 'Observação (Bloco V)';
            default: return 'Registro PDI';
        }
    };

    if (loading) return <div className="p-8 text-center text-slate-400 text-xs font-bold uppercase">Carregando PDI...</div>;

    return (
        <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
            <div className="p-6 bg-slate-50 border-b border-slate-100">
                <h3 className="text-lg font-black text-slate-800 uppercase italic tracking-tight">PDI Digital: {studentName}</h3>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Linha do Tempo de Acompanhamento</p>
            </div>
            ...
            <div className="p-6 relative">
                ...
                <div className="space-y-8">
                    {records.length === 0 ? (
                        <p className="text-center text-slate-400 text-sm py-8">Nenhum registro encontrado neste PDI ainda.</p>
                    ) : (
                        records.map((record) => (
                            <div key={record.id} className="relative pl-12">
                                ...
                                <div className="bg-white border border-slate-100 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow">
                                    <div className="flex justify-between items-start mb-2">
                                        <div>
                                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{getTitle(record.type)}</p>
