...
export const AdminPanel: React.FC = () => {
  ...
  const handleDeleteUser = async (user: UserProfile) => {
    if (
      !confirm(`Tem certeza que deseja EXCLUIR o usuário ${user.email}? Essa ação é irreversível.`)
    )
      return;
    try {
      ...
      console.warn(
        '[AdminPanel] ⚠️ Auth user NOT deleted from Supabase Auth. Profile data removed only.'
      );

      alert(
        'Dados do usuário removidos. Nota: o login do usuário ainda existe no sistema de autenticação e deve ser removido pelo Supabase Dashboard.'
      );
      loadUsers();
    } catch (e: any) {
      alert(e.message);
