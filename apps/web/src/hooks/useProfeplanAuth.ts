    ...
    const handleLogin = async (authSession: any) => {
      ...
      try {
        ...
        if (!profile && userEmail) {
          console.warn(
            `[Auth] 🩹 Profile not found for ID ${userId}. Searching by email (Ghost ID)...`
          );
          const { data: existing } = await getProfileByEmail(userEmail);

          if (existing) {
            console.log(
              '[Auth] 🩹 Ghost ID detectado — usando perfil encontrado por email (sem alterar a PK).'
            );
        ...
        if (!cancelled) {
          setSession((prev) => {
            const isIdentical =
              prev?.id === newSession.id &&
              prev?.role === newSession.role &&
              prev?.accessLevel === newSession.accessLevel;
            if (isIdentical) {
