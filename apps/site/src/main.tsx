    ...
    <main>
      <header className="nav">
        ...
        <nav aria-label="Navegação principal">
          ...
          <a href="#seguranca">IA responsável</a>
          <a href={`mailto:${CONTACT_EMAIL}`} className="nav-cta">Solicitar demonstração</a>
        </nav>
      </header>

      <section id="top" className="hero section-shell">
        <div className="hero-copy">
          <div className="eyebrow"><Sparkles size={16} /> Inteligência Artificial para a Educação Básica</div>
          <h1>Menos burocracia, mais tempo para ensinar.</h1>
          <p className="hero-lead">
            O ProfePlan apoia professores da Educação Básica na organização de planejamentos,
            planos de aula, avaliações e rotina pedagógica com Inteligência Artificial — sempre
            com revisão e decisão do professor.
          </p>
          <div className="hero-actions">
            <a className="button primary" href={`mailto:${CONTACT_EMAIL}?subject=Demonstração%20ProfePlan`}>
              Solicitar demonstração <ArrowRight size={18} />
            </a>
            <a className="button secondary" href={`${APP_URL}/login`}>
              Acessar plataforma
            </a>
          </div>
          <div className="trust-row" aria-label="Princípios do ProfePlan">
            <span><CheckCircle2 size={16} /> Professor no centro</span>
            <span><LockKeyhole size={16} /> Dados protegidos</span>
            <span><Layers3 size={16} /> Fluxo pedagógico</span>
          </div>
        </div>
          ...
          <div className="mock-dashboard">
            <aside>
              ...
              {[CalendarRange, BookOpen, FileCheck2, Brain].map((Icon, index) => (
                <div className="side-icon" key={index}><Icon size={18} /></div>
              ))}
            </aside>
            <div className="mock-content">
              ...
              <div className="mock-grid">
                <div><CalendarRange size={22} /><strong>Planejamento</strong><small>Trimestral</small></div>
                <div><PenLine size={22} /><strong>Plano de aula</strong><small>Editável</small></div>
                <div><ClipboardList size={22} /><strong>Avaliação</strong><small>Com revisão</small></div>
                <div><MessageCircleQuestion size={22} /><strong>Assistente</strong><small>IA de apoio</small></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="pain section-shell">
        <div className="section-heading">
          ...
          <p>
            Planejamentos, avaliações, registros, relatórios e adaptações fazem parte da rotina.
            O problema é quando tudo fica fragmentado, repetitivo e difícil de organizar.
          </p>
        </div>
        <div className="pain-grid">
          {['Planejamento fora do horário', 'Registros e relatórios', 'Avaliações e gabaritos', 'Adaptações pedagógicas'].map((item) => (
            <article key={item}><CheckCircle2 size={20} /><span>{item}</span></article>
          ))}
        </div>
      </section>
      ...
      <section id="como-funciona" className="workflow section-shell">
        <div className="section-heading left">
          ...
          <p>
            O ProfePlan apoia a criação de rascunhos e estruturas pedagógicas. A versão final
            sempre passa pela leitura, adaptação e decisão do professor.
          </p>
        </div>
      ...
      <section id="seguranca" className="safety section-shell">
        <div className="safety-card">
          <div>
            ...
            <p>
              O ProfePlan não substitui o professor. Ele apoia a organização do trabalho
              pedagógico com cuidado, clareza e supervisão humana.
            </p>
          </div>
          <ul>
            {safety.map((item) => (
              <li key={item}><ShieldCheck size={18} /> {item}</li>
            ))}
          </ul>
        </div>
      </section>
      ...
      <section className="audience section-shell">
        ...
        <div className="audience-grid">
          <article><GraduationCap size={24} /> Professores</article>
          <article><Users size={24} /> Coordenadores</article>
          <article><ShieldCheck size={24} /> Inclusão e apoio</article>
          <article><ClipboardList size={24} /> Gestão pedagógica</article>
        </div>
      </section>
