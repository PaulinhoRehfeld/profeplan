...
export function toPersistenceError(
  ...
  if (code === "23503" || code === "23514" || code === "23502") {
    return new KnowledgeFactoryPersistenceError(
      "CONSTRAINT_VIOLATION",
      operation,
    );
  }
  ...
  if (
    status === 401 ||
    code === "PGRST301" ||
    /missing.*identity|jwt.*missing/.test(text)
  ) {
  ...
  if (
    ...
    /network|fetch failed|failed to fetch|timeout|timed out|connection/.test(
      text,
    )
  ) {
...
export function invalidPersistenceResponse(
  operation: string,
): KnowledgeFactoryPersistenceError {
