...
integration(
  'C.2.2 stages and discards synthetic bytes in disposable Supabase Storage only',
  async () => {
    ...
    try {
      ...
      const unauthorizedUpload = await anon.storage
        .from(bucketName)
        .upload('unauthorized/synthetic.pdf', bytes, {
      ...
      const afterDiscard = await admin.storage
        .from(bucketName)
        .list(`runs/${encodeURIComponent(runA.id)}/artifacts`, {
          search: encodeURIComponent(artifactId),
          limit: 2,
      ...
      assert.equal(
        afterDiscard.data.some((item) => item.name === encodeURIComponent(artifactId)),
        false
      );
