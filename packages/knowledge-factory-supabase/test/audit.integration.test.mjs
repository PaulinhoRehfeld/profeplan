...
function assertEquivalentEvent(actual, expected) {
  const { occurredAt: actualOccurredAt, ...actualFields } = actual;
  const { occurredAt: expectedOccurredAt, ...expectedFields } = expected;

  assert.deepEqual(actualFields, expectedFields);
  assert.equal(Date.parse(actualOccurredAt), Date.parse(expectedOccurredAt));
}
...
test('AuditRepository persists, reads and isolates aggregates on disposable Supabase', async () => {
  ...
  assertEquivalentEvent(aggregateAEvents[0], EVENT_A1);
  assertEquivalentEvent(aggregateAEvents[1], EVENT_A2);
  assert.equal(aggregateBEvents.length, 1);
  assertEquivalentEvent(aggregateBEvents[0], EVENT_B1);
