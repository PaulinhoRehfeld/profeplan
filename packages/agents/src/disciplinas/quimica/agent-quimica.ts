...
export class AgentQuimica extends BaseDisciplineAgent {
  ...
  protected async _callLLM(
    ...
    return JSON.stringify({
      ...
      resposta_mock: `[MOCK] Conteúdo gerado por Lavoisier (Química) para ${String(params.tema ?? 'tema não especificado')}.`,
    });
  }
  ...
  private _loadPrompt(filename: string): string {
    ...
    return `[PROMPT: ${filename}] Conteúdo do prompt para Química.`;
  }
}
