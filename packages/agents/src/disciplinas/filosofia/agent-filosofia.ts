// ============================================================================
// PROFEPLAN — Agent Filosofia (Sócrates) — S5-01
// Agente de disciplina concreto para Filosofia — EM (somente Ensino Médio)
// ============================================================================
 ...
 *
 * Foco: história da filosofia (antiga, medieval, moderna, contemporânea),
 * ética, filosofia política, epistemologia, estética, lógica,
 * pensamento crítico e preparação para o ENEM (Ciências Humanas).
 *
 * REGRA CRÍTICA: PROIBIDO INVENTAR CITAÇÕES FILOSÓFICAS.
 * Toda citação, aforismo ou referência a obra DEVE estar confirmada na base RAG.
 */
export class AgentFilosofia extends BaseDisciplineAgent {
  /** Nome icônico do agente: "Sócrates" (homenagem a Sócrates de Atenas, "Pai da Filosofia Ocidental"). */
  public get displayName(): string {
    return AGENT_DISPLAY_NAMES[DisciplinaNome.FILOSOFIA]; // "Sócrates"
  }

  /**
   * Constrói o system prompt para Filosofia.
   *
   * Filosofia é disciplina EXCLUSIVA do Ensino Médio no currículo brasileiro.
  ...
  /** Disciplina associada a este agente: FILOSOFIA. */
  public getDisciplina(): DisciplinaNome {
    return DisciplinaNome.FILOSOFIA;
  }

  /**
   * Habilidades BNCC prioritárias para Filosofia (Ensino Médio).
   *
   * A Filosofia está inserida na área de Ciências Humanas e Sociais
   * Aplicadas (CHS). As habilidades cobrem:
   * - EM13CHS101 a EM13CHS106: Tempo e Espaço (diacronia/sincronia filosófica)
   * - EM13CHS201 a EM13CHS206: Territórios e Fronteiras (ética, alteridade)
   * - EM13CHS301 a EM13CHS306: Indivíduo, Natureza e Cultura (epistemologia, antropologia filosófica)
   * - EM13CHS401 a EM13CHS406: Política, Ética e Cidadania (filosofia política)
   * - EM13CHS501 a EM13CHS506: Relações de Poder e Trabalho (filosofia social)
   * - EM13CHS601 a EM13CHS606: Cultura, Identidade e Diversidade (estética, filosofia da cultura)
   */
  ...
  protected async _buildRagContext(tipo: TipoGeracao): Promise<string> {
    ...
    return `[RAG] Contexto de Filosofia para geração do tipo: ${tipo}. Nível: ${this.context.nivel}.`;
  }
  ...
  protected async _callLLM(
    ...
    return JSON.stringify({
      ...
      resposta_mock: `[MOCK] Conteúdo gerado por Sócrates (Filosofia) para ${params.tema || 'tema não especificado'}.`,
    });
  }
  ...
  private _loadPrompt(filename: string): string {
    ...
    return `[PROMPT: ${filename}] Conteúdo do prompt para Filosofia.`;
  }
}
