...
const CONTRIBUTION_CONTRACT_VERSION: KnowledgeContributionCandidate['contractVersion'] = '1.0.0';
...
export class KnowledgeContributionService {
  distill(request: KnowledgeContributionRequest): readonly KnowledgeContributionCandidate[] {
    if (request.structuralReview.candidateSnapshotId !== request.reconstruction.snapshotId) {
      throw new Error('structural review does not belong to the supplied reconstruction candidate');
    }

    const elementById = new Map(
      request.reconstruction.elements.map((element) => [element.elementId, element])
    );
    const confirmedElementIds = new Set(
      request.structuralReview.decisions
        .filter(
          (decision) => decision.targetKind === 'element' && decision.disposition === 'confirmed'
        )
    ...
    return request.proposals.map((proposal) => {
      ...
      if (!proposal.statement.trim()) {
        throw new Error(`statement is required for contribution ${proposal.contributionId}`);
      }
      if (proposal.sourceElementIds.length === 0) {
        throw new Error(`source elements are required for contribution ${proposal.contributionId}`);
      }

      const sourceElements = proposal.sourceElementIds.map((elementId) => {
        const element = elementById.get(elementId);
        if (!element) {
          throw new Error(`source element not found in reconstruction candidate: ${elementId}`);
        }
        if (!confirmedElementIds.has(elementId)) {
          throw new Error(`source element is not structurally confirmed: ${elementId}`);
        }
        return element;
      });
