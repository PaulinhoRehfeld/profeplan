    ...
    return (
        <div className="p-6 max-w-6xl mx-auto space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Relatório de Aprimoramento</h1>
                    <p className="text-slate-500 text-sm">Baseado nas observações dos professores (Loop de Feedback)</p>
                </div>
                <button
                ...
                {feedbacks.map(item => (
                    <div key={item.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all">
                        <div className="flex items-start justify-between mb-3">
                            <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wide ${item.feature === 'term_planning' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700'}`}>
                        ...
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <p className="text-xs font-bold text-indigo-900 uppercase">Solicitação de Ajuste (Feedback)</p>
                            </div>
