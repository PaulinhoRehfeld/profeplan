                ...
                <div>
                    ...
                    <div className="flex gap-2 mt-2">
                        {student.deficiencies?.map(tag => (
                            <span key={tag} className="text-[9px] font-black uppercase tracking-wider bg-slate-200 text-slate-600 px-2 py-1 rounded">
                                {tag}
                            </span>
                        ))}
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between mt-6 pt-4 border-t border-slate-50 gap-4">
                        <div className="flex items-center gap-2">
                            <label className={`w-full sm:w-auto flex items-center justify-center gap-2 cursor-pointer px-6 py-3 rounded-xl border transition-all ${adaptation.status === 'validated' ? 'bg-green-50 border-green-200 text-green-700' : 'bg-white border-slate-200 hover:bg-slate-50'}`}>
                                ...
                                />
                                {adaptation.status === 'validated' ? <CheckCircle2 size={16} /> : <div className="w-4 h-4 border-2 border-slate-300 rounded-full"></div>}
                                <span className="text-[10px] font-black uppercase tracking-widest">
                                    {adaptation.status === 'validated' ? 'Validado para Relatório' : 'Validar Adaptação'}
                                </span>
                            </label>
                        </div>
                        <div className="flex gap-2">
                            <button
                                onClick={() => onGenerate(student)}
                                className="p-2 text-slate-400 hover:text-teal-600 hover:bg-teal-50 rounded-lg transition-colors" title="Regerar"
                            >
                                <RefreshCw size={16} />
                            </button>
