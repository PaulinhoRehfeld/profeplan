        ...
        <div className="space-y-8">
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 flex items-start gap-3">
                ...
                <div className="text-sm text-yellow-900">
                    <strong>Modo Visualização:</strong> Você pode visualizar estes dados, mas apenas o gestor escolar pode editá-los.
                </div>
            </div>
            ...
            >
                ...
                <InfoRow label="Diagnóstico Clínico" value={data.bloco_1_identificacao?.diagnostico_clinico} multiline />
            ...
            >
                <InfoList label="Necessidades Específicas" items={data.bloco_2_diagnostico?.necessidades_especificas} />
                <InfoList label="Áreas Comprometidas" items={data.bloco_2_diagnostico?.areas_comprometidas} />
                ...
                <InfoRow label="Medicamentos em Uso" value={data.bloco_2_diagnostico?.medicamentos_uso} multiline />
                <InfoRow label="Restrições de Atividades" value={data.bloco_2_diagnostico?.restricoes_atividades} multiline />
            </BlockSection>
            ...
            >
                ...
                <InfoList label="Objetivos Específicos" items={data.bloco_3_objetivos?.objetivos_especificos} />
            ...
            >
                <InfoList label="Recursos Tecnológicos" items={data.bloco_4_recursos?.recursos_tecnologicos} />
                <InfoList label="Materiais Adaptados" items={data.bloco_4_recursos?.materiais_adaptados} />
                <InfoRow label="Mobiliário Específico" value={data.bloco_4_recursos?.mobiliario_especifico} multiline />
            ...
            >
                ...
                {data.bloco_5_equipe?.equipe_multidisciplinar && data.bloco_5_equipe.equipe_multidisciplinar.length > 0 && (
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Equipe Multidisciplinar</label>
                        <div className="space-y-2">
                            {data.bloco_5_equipe.equipe_multidisciplinar.map((membro, index) => (
                                <div key={index} className="bg-slate-50 p-3 rounded-lg">
                                    ...
                                    {membro.contato && (
                                        <p className="text-sm text-slate-500">{membro.contato}</p>
                                    )}
                                </div>
                            ))}
                        </div>
            ...
            >
                <InfoRow label="Frequência de Atendimento" value={data.bloco_6_atendimento?.frequencia_atendimento} />
            ...
            >
                <InfoRow label="Responsável Principal" value={data.bloco_7_familia?.responsavel_principal} />
                <InfoRow label="Contato do Responsável" value={data.bloco_7_familia?.contato_responsavel} />
                <InfoRow label="Participação da Família" value={data.bloco_7_familia?.participacao_familia} multiline />
                <InfoRow label="Orientações para a Família" value={data.bloco_7_familia?.orientacoes_familia} multiline />
            </BlockSection>
            ...
            >
                <InfoRow label="Observações Gerais" value={data.bloco_8_observacoes?.observacoes_gerais} multiline />
                <InfoRow label="Histórico Escolar" value={data.bloco_8_observacoes?.historico_escolar} multiline />
                <InfoRow label="Transferências" value={data.bloco_8_observacoes?.transferencias} multiline />
                <InfoRow label="Outras Informações" value={data.bloco_8_observacoes?.outras_informacoes} multiline />
            </BlockSection>
        </div>
    );
};
...
const InfoRow: React.FC<{
    ...
    return (
        <div>
            ...
            {multiline ? (
                <div className="bg-slate-50 p-3 rounded-lg text-slate-900 whitespace-pre-wrap">
                    {value}
                </div>
            ) : (
