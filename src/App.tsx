...
import SettingsModal from './components/SettingsModal'; // [FIX] Eager Load to prevent 404s
...
// const SettingsModal = React.lazy(() => import('./components/SettingsModal')); // Switched to Eager
    ...
    if (session?.id) {
      ...
      const initSession = async () => {
        ...
        if (!profileData && session.email === 'prehfeld@hotmail.com') {
          ...
          profileData = {
            id: session.id, // Use session ID to satisfy types
            email: session.email,
            role: 'admin',
            is_admin: true,
            tier: 'GOLD',
        ...
        if (profileData) {
          ...
          // Used 'isHardcodedAdmin' calculated above
                    ...
                    <div className="app-container flex h-screen bg-slate-50 overflow-hidden font-sans">
                      <Sidebar
                        ...
                        onLogout={async () => {
                          ...
                          // Local Clear FIRST to prevent UI hang
                          setSession(null);
                          setUserProfile(null);
                          localStorage.removeItem('profeplan_session');
                          localStorage.removeItem('supabase_user_id');
                          ...
                          try {
                            ...
                            // Ignore network errors on signout, we are already "out" locally
                            console.warn("Supabase SignOut incomplete (likely network/token issue):", err);
                          }
                        ...
                        <header className="h-16 bg-white/90 backdrop-blur-xl border-b border-slate-100 flex items-center justify-between px-4 md:px-6 z-50 sticky top-0 shadow-sm">
                          <div className="flex items-center gap-4">
                            ...
                            <div className="flex flex-col">
                              <h2 className="font-black text-slate-900 tracking-tighter uppercase italic text-lg leading-none">PROFEPLAN V3.6</h2>
                      ...
                      <aside className={`h-screen bg-white border-l border-slate-100 shrink-0 lg:flex lg:flex-col lg:w-64 p-6 space-y-6 overflow-y-auto ${activeMode === ToolMode.QUARTERLY_PLANNING ? 'hidden' : 'hidden lg:flex'}`}>
                        {customSidebar ? (
                          ...
                          <div>
                            <h3 className="font-black text-[10px] uppercase tracking-[0.2em] text-slate-400 italic mb-8">PROFEPLAN V3.6</h3>
                      ...
                      {/* Settings Modal (Eager Loaded - Conditional Mount) */}
                      {isSettingsOpen && (
                        <SettingsModal
