...
O deploy Vercel atual encaminha `/api/*` ao diretório raiz `api/`. `apps/bff` não foi identificado como composition root Supabase consolidado nesta inspeção.
...
- o Lote 3B não migra backend para outro runtime;
...
`@profeplan/logger` possui contexto estruturado, mas também usa filesystem síncrono; rotas Vercel atuais usam loggers inline/console em alguns pontos.
