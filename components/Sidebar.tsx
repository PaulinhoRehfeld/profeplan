...
import { LayoutDashboard, BookOpen, PenTool, Accessibility, FileText, Settings, MessageSquare, ShieldCheck, X, Crown, FolderClosed } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ activeMode, setActiveMode, onOpenSettings, isOpen, onClose, userRole }) => {
  const menuItems = [
    { id: ToolMode.CHAT, icon: MessageSquare, label: 'Assistente (Chat)' },
      ...
      <div className={`w-64 bg-slate-900 h-screen text-slate-300 flex flex-col fixed left-0 top-0 z-[70] transition-transform duration-300 transform ${
        ...
        <div className="p-6">
          ...
          <nav className="space-y-1">
            {menuItems.map((item) => (
              ...
              >
                <item.icon className={`w-5 h-5 ${activeMode === item.id ? 'text-blue-400' : 'text-slate-500 group-hover:text-slate-300'}`} />
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group ${
                    activeMode === ToolMode.ADMIN 
                      ? 'bg-amber-600/10 text-amber-400 border-r-4 border-amber-600' 
                      : 'hover:bg-slate-800 hover:text-white'
                  }`}
                >
                  <Crown className={`w-5 h-5 ${activeMode === ToolMode.ADMIN ? 'text-amber-400' : 'text-slate-500 group-hover:text-slate-300'}`} />
        ...
        <div className="mt-auto p-6 border-t border-slate-800">
          ...
          <button 
            ...
            className="flex items-center gap-3 px-4 py-3 w-full rounded-xl hover:bg-red-900/20 transition-colors text-slate-400 hover:text-red-400 mt-1"
          >
            <X className="w-5 h-5" />
