...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!deepseek) {
    return res
      .status(500)
      .json({ error: 'DEEPSEEK_API_KEY ou OPENAI_API_KEY não configurada no servidor.' });
  }
    ...
    switch (action) {
      case 'generateStudentAdaptation': {
        ...
        if (!originalContent || !studentName || !gradeLevel) {
          return res
            .status(400)
            .json({ error: 'Missing parameters for generateStudentAdaptation' });
        }
        systemInstruction =
          'Você é um especialista em inclusão e DUA escrevendo adaptações de aula.';
...
RETORNE EM MARKDOWN: ## 🎯 Objetivos Adaptados, ## 🛠️ Estratégias de Acesso, ## 📝 Atividade Adaptada, ## 📏 Avaliação Diferenciada.`;
      ...
      case 'generatePdiReport': {
        ...
        systemInstruction =
          'Você é um especialista em educação especial redigindo relatórios oficiais de PDI.';
...
TAREFA: Redija um relatório final do PDI em 3-5 parágrafos, texto corrido, tom formal e acolhedor, pronto para impressão oficial.`;
      ...
      case 'generateBlock9Adaptation': {
        ...
        isJson = true;
        systemInstruction =
          'Você é especialista em inclusão escolar e DUA gerando adaptações curriculares. Retorne APENAS JSON.';
...
Retorne APENAS JSON:
{
  ...
  "tempo_estimado": "ex: 50 minutos"
      ...
      case 'generateBlock10Diagnosis': {
        ...
        isJson = true;
        const diagnostico =
          fullPdiContext.block_1_8?.bloco_1_identificacao?.diagnostico_clinico ||
          'Não especificado';
        const necessidades =
          fullPdiContext.block_1_8?.bloco_2_diagnostico?.necessidades_especificas || [];
        const potencialidades =
          fullPdiContext.block_1_8?.bloco_2_diagnostico?.potencialidades || [];
        ...
        systemInstruction =
          'Você é especialista em avaliação pedagógica e educação inclusiva. Retorne APENAS JSON.';
  ...
  "ia_projecao_proximo_periodo": "projeção narrativa (50-100 palavras)"
    ...
    if (isJson) {
      ...
      try {
        ...
        return res
          .status(200)
          .json({ raw: result, error: 'Failed to parse JSON from AI response' });
      }
    }
