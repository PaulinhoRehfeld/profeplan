...
for row in sheet.iter_rows(min_row=10, values_only=True):
    ...
    # SQL SIMPLIFICADO (SEM DO $$)
    sql = f"UPDATE schools SET inep_code = '{inep_clean}' WHERE UPPER(TRIM(name)) = '{name_norm}' AND UPPER(TRIM(city)) = '{city_norm}' AND inep_code IS NULL AND NOT EXISTS (SELECT 1 FROM schools WHERE inep_code = '{inep_clean}');"
...
for i in range(total_batches):
    ...
    with open(output_dir / f"final_batch_{i+1:02d}.sql", 'w', encoding='utf-8') as f:
        f.write("\n\n".join(batch))

print(f"✅ Gerados {total_batches} lotes simplificados em: {output_dir}")
